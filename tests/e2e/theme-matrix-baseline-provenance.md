...
## 2026-09-19 — **이 기계의 OS 가 올라가 baseline 거의 전부가 붉다. 재생성하지 않았다**

**측정.** `snapshot-environment.json` 이 적는 채취 환경은 `darwin 25.6.0` 이고 이 기계는 지금 `darwin 27.0.0` 이다. `npm run test:e2e:gate` 가 **123 중 6 통과 · 117 실패**이며, 실패는 전부 글자 위 서브픽셀 차이다(대표값: `mobile-test-question en light` 가 **42 px · 전체의 0.01**). diff 이미지에서 차이가 특정 원소가 아니라 **모든 글리프에 흩어져** 있다 — 내용이 아니라 래스터화가 달라진 모양이다.

**원인을 코드와 갈랐다.** 같은 clone 에서 `origin/main` 으로 되돌려 다시 빌드하고 같은 케이스를 돌렸더니 **똑같이 42 px 로 붉었다.** 이어서 양쪽의 실패 목록을 정렬해 비교했다 — `main` 117 · 작업 브랜치 117 이고 **집합이 완전히 같다**. 즉 그 붉음은 어떤 변경의 결과가 아니라 이 기계의 환경이다.

**재생성하지 않은 이유.** 이것은 기능 변경이 아니라 **환경 이관**이다. 180 여 장을 기능 단위와 같은 커밋에 섞으면 그 단위의 시각 델타가 0 이라는 사실이 diff 안에서 읽히지 않게 되고, 단위 11 이 그랬듯 재생성은 제 단위를 갖는 편이 맞다. 그리고 이 표가 요구하는 것은 「무엇이 왜 바뀌었나」인데, 환경 이관의 답은 「코드는 아무것도 바뀌지 않았다」이므로 그 자체가 따로 적혀야 한다.

**다음 세션이 할 일.** 재생성은 `qa:visual:full` 하나로 끝나지 않는다(`safari-hover-ghosting.spec.ts` 의 PNG 도 본다). 함께 `snapshot-environment.json` 의 `release` 를 `27.0.0` 으로 갱신한다 — 그 파일이 낡은 채로 재생성하면 다음 기계가 같은 조사를 처음부터 반복한다.
