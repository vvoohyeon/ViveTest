# CLAUDE.md — ViveTest Coding Agent Instructions (Claude Code)

## 0. Scope and authority

이 파일은 ViveTest 저장소의 **프로젝트 계약(what this repo requires)** — Claude Code용 정본이다. Codex는 동일 계약의 Codex 정본인 `AGENTS.md`를 따른다. 두 파일은 쌍둥이 정본이며, 계약 사실(경로·명령·경계·anchor)이 바뀌면 같은 변경에서 함께 갱신한다.

- **적용:** 코드·테스트·리팩터링·개발 문서·저장소 설정 변경. 이 저장소에는 런타임 에이전트가 없다 — 이 파일이 곧 코딩 계약이다.
- **비적용:** *how to work*(세션 시작·명확화·plan mode·skill·검증 절차·STATE.md·권한/승인 판단)는 전역 오케스트레이션(`~/.claude/CLAUDE.md`)의 몫이다 — 여기 재진술하지 않는다.
- **우선순위:** 호스트·사용자 지시 > 이 프로젝트 계약 > 전역 기본. 저장소 고유 사항은 이 파일이 전역 기본을 override한다. 더 가까운 하위 `AGENTS.md`는 델타만 추가하며 이 파일의 불변 규칙·승인 경계를 완화할 수 없다.
...
> *surgical changes*, *minimum code*, *read before asserting*, *verifiable completion* 등 **도구 무관 작업 규율은 전역 `~/.claude/CLAUDE.md` 소관**이다 — 여기 재진술하지 않는다.
...
rebuild 작업에서 확정된 branch/worktree/checkpoint 토폴로지를 재설계·개명·재해석하지 않는다 — 추론으로 재구성하지도 않는다. 토폴로지는 고정된 프로젝트 사실이다.
...
- **지침 파일 갱신 트리거**(승인된 규칙 개정 요청 하에서만): 명령·script 변경 / 프로젝트 사실(route·locale·stack·anchor·생성-SSOT 경계) 변경 / 소유권·구조·gold standard 변경 / repo 고유 실수 2회 이상 반복. 갱신 시 파일 경로·명령·locale·anchor를 실제 저장소와 대조하고, 계약 사실이 바뀌면 **쌍둥이 `AGENTS.md`를 같은 변경에서 함께 갱신**한다.

## 10. Claude Code guardrails (repo-specific)

Claude Code의 자율 편집 실패 모드를 이 저장소 맥락에서 막는 규칙이다.

- **계약을 기억으로 인용하지 않는다.** 규칙·근거를 설명하거나 그에 따라 행동하기 전에 §2가 라우팅하는 해당 섹션을 다시 읽는다.
- **rebuild 편집 전 자기 위치·wave 경계 확인.** 편집 의도 작업에서 현재 workspace가 확정 활성 구현 공간인지 확인한다 — `legacy/reference`·checkpoint worktree 안이면 파일 수정 전에 정지한다. 요청이 해당 wave의 include/exclude 경계를 넘으면 계획 작성 전에 정지·보고한다.
...
- **인접 개선은 제안으로만.** 요청 밖에서 발견한 개선·정리 거리는 적용하지 않고 제안으로 보고한다. 안정 모듈의 스타일·구조 재작성은 명시 요청 시에만.
- **행동 계약이 불확실하면 가정하기 전에 기존 E2E 커버리지를 먼저 확인한다.**
- **여러 파일을 수정할 때는 실행 순서를 먼저 밝히고 진행한다.**
- **루트·하위 지침 충돌 시 자체 해소 금지.** 파일 경로·명령·계약 참조가 루트와 하위 `AGENTS.md`(또는 이 파일) 간에 다르면 정지 — 충돌 항목을 출처와 함께 나열하고 문서 충돌로 보고한다.
