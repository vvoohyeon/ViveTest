...
## 14. 미확인 — 다음 세션이 먼저 재야 할 것

이 분석이 **재지 못한 것**이다. 결함보다 이쪽이 값지다 — 모르는 것을 안다고 적지 않았다는 뜻이기 때문이다.

| # | 미확인 | 왜 못 쟀나 | 어떻게 닫나 |
|:--:|:---|:---|:---|
| 1 | **실기기 성능** | 측정이 전부 로컬 루프백이라 네트워크가 없다. LCP 116ms · CLS 0.0000 · 폰트 2,009KB 가 12ms 에 온 것은 실기기 값이 아니다 | 실제 iOS/Android 기기 또는 스로틀링된 프로파일에서 다시 잰다. 구조 사실(폰트 바이트·`font-display: swap`·preload 부재)만으로 판단하고 있는 현재 상태를 수치로 바꾼다 |
| 2 | **주소창 높이 변동 중의 고정 요소** | 헤드리스 브라우저는 모바일 브라우저 크롬을 흉내 내지 않는다 | 실기기에서 스크롤하며 GNB·동의 배너·유령 GNB 의 어긋남을 본다. `dvh` 결함(blocker)의 실제 체감이 여기서 확정된다 |
| 3 | **스크린리더 실사용** | ARIA 속성의 유무는 쟀으나 VoiceOver/TalkBack 이 실제로 무엇을 읽는지는 재지 않았다 | 카드 확장·드로어·instruction 다이얼로그를 실제 스크린리더로 훑는다 |
| 4 | **저사양 CPU** | 스로틀링 없이 쟀다. 제목 연속성의 이분 탐색과 그리드 지오메트리의 강제 레이아웃 비용이 프레임 예산 안인지가 서명 인터랙션 6·7 의 「결정 필요」를 가른다 | CPU 4× 스로틀에서 확장 1 회의 레이아웃 비용을 잰다 |
| 5 | **`.motionStageLate` 가 죽은 것이 의도인가** | `design.md` 가 3 단 스태거를 규정하는지 확인하지 못했다 | `design.md` §8 을 읽고, 규정한다면 step 3 에서 되살릴 대상이고 아니면 step 1 에서 지운다 |
| 6 | **행 베이스라인 동결이 여전히 필요한가** | 데스크톱이 이미 오버레이로 흐름에서 빠지는데 동결이 왜 남아 있는지 문서로 확인되지 않았다 | 동결을 끄고 시각 baseline 을 돌리면 한 번에 판정된다 |
| 7 | **모바일 뒤로가기 220ms 홈 폴백의 계약 근거** | 어느 조항이 이것을 요구하는지 찾지 못했다. iOS 스와이프와의 충돌 여부도 미확인 | 두 미확인을 닫아야 서명 인터랙션 16 의 판정이 선다 |
| 8 | **비평 발굴 27행** | 적대적 검증을 거치지 않았다 | 착수 전에 재현한다 |
