...
## 15. 확정된 제품 결정 (2026-09-14)

**이 열둘은 확정이다. 새 세션은 다시 묻지 않는다.**

| # | 결정 | 근거 |
|:--:|:---|:---|
| 1 | **모바일 확장을 바텀시트로 교체한다** | 사용자 결정. 삭제되는 복잡도가 가장 크고(스냅샷·복원 폴링·transient 껍데기 등 약 500행), 접근성 결함 다섯이 패턴 교체 한 건으로 닫히며, 데스크톱 계약을 건드리지 않는다 |
| 2 | **동의 UI 는 바텀시트다 — 모달 시트가 아니다** | 사용자 지시. 「랜딩 직후 모달로 화면 전체를 덮어 사용자가 부담을 느끼고 이탈하지 않도록」이 목적이다. 바텀시트는 하단 일부만 덮고 **뒤에서 카탈로그를 계속 스크롤할 수 있어야 한다**(비모달) |
| 3 | **instruction 화면의 모달 시트는 유지한다** | 사용자 지시. 동의 미선택 상태로 테스트에 진입할 때는 선택을 강제하는 것이 맞다 |
| 4 | **카드 탭 시 동의 바텀시트가 사전질문 바텀시트로 교체돼도 좋다** | 사용자 승인. **단 같은 UI 요소를 두 기능이 공유하므로 전환 인터랙션이 아주 정교해야 한다** — 시트 컨테이너는 하나로 두고 내용만 교차 페이드하며, 높이 변화는 단일 스프링 없는 전이로 잇는다. 두 시트가 동시에 존재하는 프레임이 한 장도 없어야 한다 |
| 5 | **랜딩 밀도는 중간안 — 썸네일만 조정한다** | 사용자 결정. 1 열을 유지하고 썸네일 16:6 → 16:4, 부제 모바일 clamp 도입. 약 3.0 화면. **2 열 그리드는 배제한다** — 좁은 폭을 다시 가르는 것이 시각 균형과 정보 밀도 양쪽에 불리하다는 판단 |
| 6 | **폰트는 로케일별 subset + preload** | 사용자 결정. Slow 4G 10.9 초의 지배 요인이 폰트 하나다. 현재 locale 것만 preload 하고, 12 locale 전부 Pretendard 로 렌더되며 fallback 이 없으므로 **커버리지를 잃지 않는 subset 이어야 한다** |
| 7 | **결과 URL 과 동적 OG 를 이번 리팩터에 넣는다** | 사용자 결정. 심리 테스트의 유통 경로가 메신저 링크이고 그 링크가 휴대폰에서 열린다. 도메인 층은 이미 있고 없는 것은 라우트와 인코딩 한 층뿐이다 |
| 8 | **상시 하단 탭 바를 만들지 않는다** | 화면이 일곱뿐이라 탭 바가 과하고, 하단 밴드 경쟁을 영구화한다. `no-persistent-primary-nav-on-mobile` 행은 기각하고 GNB 드로어를 유지한다 |
| 9 | **hover intent 527행은 보존한다** | 데스크톱 전용이고 주석에 실측 로그로 남은 함정 5 종이 실재한다. 결정 2 로 대상 기기가 줄지만 그 기기들에서는 여전히 필요하다 |
| 10 | **제목 연속성 301행은 보존한다** | §14 #4 의 측정. 1×/4× 예산 안이고 hover 가능 기기에만 존재한다 |
| 11 | **랜딩 키보드 진입은 skip link 로 교체한다** | 표준 대안이 정확히 존재하고, 현재 구현이 스스로 계층 위반을 인정한다(`use-landing-keyboard-entry.ts:17-29` 의 `@future-move R-06`). 탭 순서를 상태에 따라 바꾸는 것은 예측 가능성을 해친다. `req-landing.md` §7.6:480-481 개정 |
| 12 | **유령 GNB 는 이번 범위 밖이다** | View Transition 공유 요소로의 교체는 명확한 대안이지만 전환 계약 전체를 건드리므로 별도 단위다. 보존하고 다음으로 넘긴다 |
