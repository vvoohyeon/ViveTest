...
**이 열여섯은 확정이다. 새 세션은 다시 묻지 않는다.** 시각·인터랙션의 구체는 `docs/plans/2026-09-14-mobile-refactor-design-spec.md` 가 갖는다 — 그 문서가 이 표의 아래에 있는 모든 「어떻게」의 정본이다. 14~16 과 5 의 개정은 2026-09-14 외부 리뷰어 관점 발굴을 사용자가 채택한 것이다.
...
| 5 | **instruction 은 모달 바텀시트다**(2026-09-14 개정 — 종전 「현행 유지」) | 마찰 사다리의 마지막 칸이고 선택을 강제하는 것은 그대로다. 바뀌는 것은 형태다 — 전면 시트는 본문과 CTA 사이가 화면의 72% 만큼 비고 CTA 가 상단 1/4 에 있었으며 액션 행이 8/12 locale 에서 접혔다. 행동 파일 넷은 불변이고 시작 뒤 Q2 가 이어진다 |
...
| 13 | **동의 수정 경로는 얇은 텍스트 링크다** | 모바일은 드로어 설정 블록의 우측 정렬 링크, 데스크톱은 페이지 최하단 중앙, 그리고 `OPTED_OUT` 고지 행의 `동의 변경`. 누르면 **같은 배너**가 다시 뜨고 이전 선택이 표시되며 재호출 배너에만 닫기 X 가 있다 |
| 14 | **Deny 뒤의 카탈로그는 숨은 개수와 해제 경로를 그리드 상단에 고지한다** | 8 장이 2 장으로 접히는데 설명이 0 줄이었다. 필터가 목록을 줄이면 「무엇이 왜 숨었는가 + 해제 경로」를 같은 자리에 두는 것이 목록 UI 의 기본이다 |
| 15 | **시트에서 A/B 를 탭하면 곧바로 테스트로 간다** — 별도 「다음」 CTA 없음 | 데스크톱의 hover → 클릭 진입과 같은 박자이고 탭 수 2 가 유지된다. 오탭 복구는 이전-삭제 개정 뒤의 「이전」이다. 문항 화면의 자동 진행도 그대로다 — 「다음」 버튼은 어디에도 없다 |
| 16 | **랜딩 히어로 밴드를 걷고 페이지 제목만 남긴다** | `design.md` §7.1 「No hero」와 실제 렌더가 어긋나 있었다. 모바일 GNB 에 화면 이름 슬롯이 없어 `h1` 은 남겨야 하고, 밴드·큰 헤드라인·두 문장 단락은 걷는다 |
