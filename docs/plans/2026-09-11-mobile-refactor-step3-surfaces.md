...
## 0-2. step 2 가 배정받았으나 하지 않은 계약 개정 둘 — 이 단계가 가져간다
...
| 개정 | 현행 조항 | 왜 이 단계인가 |
|:---|:---|:---|
| `Esc` 가 consent 를 저장하지 않는다 | `req-test.md` §4.1 `:403` 「`Esc` 는 그 단계의 dismiss action 과 같다」 — 취소 키가 개인정보 설정을 영구 저장한다 | 시트 프리미티브가 `Esc` 를 소유하므로 §1-2 가 이 조항 위에서 구현한다 |
| 후진 이동이 응답을 지우지 않는다 | `req-test.md` §3.9 `:525-529` 제거 predicate `Number(key) < nextIndex` | 명세 §2-2 의 오탭 복구 경로가 이것을 전제한다 — 전제가 거짓이면 그 표면을 만들 수 없다 |

**둘 다 Ask-First SSOT 개정이다.** 각각 사용자 승인을 받은 뒤 해당 단위와 **같은 커밋**에서 움직인다. 개정 없이 그 표면을 먼저 만들면, 만든 것이 계약을 어기는 상태로 착지한다.
...
`req-landing.md:906` 「Desktop: centered card overlay. Mobile: full-screen overlay.」 의 뒤 절반을 「Mobile: 모달 바텀시트(명세 §2-3 · §3-4)」로 고친다. `req-test.md` §3.6 은 **한 글자도 바뀌지 않는다** — `:399` 의 「별도 route 가 아니라 `/test/{variant}` 위 overlay」는 시트도 만족하고, `:402` 의 모달 다이얼로그 요건(역할 · 제목 · 컨테이너 포커스 · Tab 순환)은 프리미티브가 그대로 충족한다. **`:403` 의 `Esc` 의미는 아직 개정되지 않았다** — step 2 가 가져갔으나 실행하지 않았고 `req-test.md` 는 이 범위에서 한 글자도 바뀌지 않았다(2026-09-15 감사). 그 조항은 여전히 「`Esc` 는 그 단계의 dismiss action 과 같다」이고, 그래서 취소 키가 consent 를 영구 저장한다(blocker `esc-aliased-to-consent-button`). 시트 프리미티브가 `Esc` 를 다루므로 **이 단위가 그 개정을 함께 가져간다** — §0-2 를 먼저 읽는다.
...
- [ ] blocker 셋(저장소 차단 · Next 폴백 · 세그먼트 404)이 닫혔다. 로케일 별칭은 step 2 와 2026-09-15 감사 후속이 이미 닫았다 — 다시 하지 않는다.
- [ ] §0-2 의 계약 개정 둘이 각각 승인을 받아 해당 단위와 같은 커밋에서 움직였다.
