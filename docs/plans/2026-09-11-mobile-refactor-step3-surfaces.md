...
| 후진 이동이 응답을 지우지 않는다 | **열려 있다.** `req-test.md` §3.9 의 「구현 계약 (backward navigation)」 제거 predicate `Number(key) < nextIndex` 가 그대로다 | 명세 §2-2 의 오탭 복구 경로가 이것을 전제한다 — 전제가 거짓이면 그 표면을 만들 수 없다 |

**열려 있는 쪽은 Ask-First SSOT 개정이다.** 사용자 승인을 받은 뒤 해당 단위와 **같은 커밋**에서 움직인다. 개정 없이 그 표면을 먼저 만들면, 만든 것이 계약을 어기는 상태로 착지한다.
