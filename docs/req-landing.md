...
- GNB 우측 테마 버튼의 Desktop hover 확장/slot anchor/고정 배열 정책 변경 시 Section 6.4, 10.2, 14.2를 동기화한다.
...
- Desktop GNB 우측 테마 버튼은 테마 버튼 전용 예외로 현재 테마 상태를 닫힌 상태에서 표시한다. 이 예외는 언어 선택 등 다른 Desktop 설정 레이어 컨트롤의 배열/동작을 변경하지 않는다.
- Desktop hover 확장 시 테마 선택지는 모든 locale에서 항상 `Dark / Light` 순서로 표시한다.
- 닫힌 상태의 테마 버튼은 GNB 우측 끝 anchor에 고정한다. 현재 테마가 `light`인 경우 닫힌 상태의 Light 버튼과 확장 상태의 Light slot은 같은 anchor를 공유해야 한다. 구현 검증 기준에서는 두 상태의 bounding box `x/y` 좌표가 `0px` 오차로 일치해야 한다.
- 현재 테마가 `dark`인 경우 닫힌 상태의 Dark 버튼은 hover 확장 시 좌측 Dark slot으로 이동해야 한다. 이동은 위치 이동이 식별되는 최소 slide 계열 motion이면 충분하며, 크기/opacity/easing 세부값은 구현 재량으로 둔다.
- Desktop hover 확장 중 현재 모드는 native `disabled` 버튼으로 표시하며 클릭/키보드 전환 target이 아니다. 반대 모드 클릭은 테마 변경을 실행하고 확장 상태를 즉시 닫은 뒤 기존 테마 전환 효과를 적용한다.
- Desktop theme control의 hover hit area는 닫힌 버튼과 확장 패널 사이에서 연속적이어야 한다. 단, visible control bounds 밖의 ghost/placeholder 영역은 pointer 이탈 방지 용도에 한정하며 클릭 동작을 제공하면 안 된다.
- Keyboard path에서는 theme control focus 후 `Enter` 또는 `Space`로 확장할 수 있으며, 닫힘은 기존 Desktop 설정 레이어 닫힘 규칙을 따른다.
...
**GNB Theme Control Notes**:
- GNB 우측 테마 버튼은 `light | dark` 2-state control로 유지한다. `system`은 초기 system-follow 상태의 저장 정책으로만 취급하며, 이번 UI 선택지에는 포함하지 않는다.
- 닫힌 상태는 현재 적용 중인 테마를 표시하고, Desktop hover 확장 상태는 `Dark / Light` 고정 배열을 표시한다.
- 현재 모드는 native `disabled` 버튼으로 표현하며, 반대 모드는 사용자가 클릭 가능한 전환 target으로 더 명확하게 인지되어야 한다.
- 현재 모드는 클릭/키보드 전환 target이 아니며 상태 변경, 닫힘, 별도 피드백을 발생시키지 않는다.
- 반대 모드 클릭 후의 실제 테마 전환 애니메이션은 기존 theme transition 구현을 재사용한다.
- Mobile/Touch의 테마 선택 배열과 표시 방식은 기존 구현을 유지한다.
...
9. GNB Theme Control: Desktop hover-capable 환경에서 theme control 확장 시 모든 locale의 slot 순서가 `Dark / Light`로 고정되고, Light 현재 상태의 닫힘 버튼과 확장 Light slot bounding box `x/y`가 `0px` 오차로 일치하며, Dark 현재 상태의 닫힘 버튼이 hover 확장 후 좌측 Dark slot에 도달하는지 검증한다. 현재 모드는 native `disabled` 버튼이며 클릭/키보드 전환 target에서 제외되어야 한다. Mobile/Touch 기존 배열은 이번 검증 범위에서 제외한다.
