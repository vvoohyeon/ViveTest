...
- opt_out 카드 진입은 §13.10 consent 게이트의 적용 대상이 아니다. consent 상태와 무관하게 게이트 없이 바로 테스트를 시작한다.

#### 구현 시점

- opt_out 카드 필터링 로직은 variant registry 인터페이스 확정(Test Flow Phase 2) 이후
  착수 가능하다. registry의 `cardType` 컬럼이 SSOT이기 때문이다.
- Phase 2 착수 전까지 opt_out 카드는 `available`과 동일하게 처리한다(임시).
...
2. Automated: consent 상태 전환 시 카탈로그 필터 결과 변경을 검증한다.
3. Automated: opt_out 카드 진입 경로가 available 카드와 동일하게 동작함을 검증한다.

### 13.10 Consent Gate at Test Entry

**핵심 목표**: available 카드는 opt_in 사용자 전용이다.
미선택 default 상태에서는 랜딩 카탈로그 클릭까지 허용하되,
실제 테스트 시작 시점에 consent를 재확인해 opt_in을 유도한다.

#### 발동 조건

아래 두 조건을 **모두** 만족할 때 consent 게이트가 발동한다.

1. 진입 카드 타입이 `available`이다.
2. 현재 consent 상태가 미선택(default)이다.
   Agree All 또는 Disagree All이 이미 결정된 사용자에게는 발동하지 않는다.

**`opt_out` 카드는 consent 게이트 발동 대상이 아니다.**
...
**게이트 표시**:
- 테스트 페이지 진입 후 instruction overlay와 함께
  Consent UI를 추가로 노출한다.
- 두 UI의 구체적인 레이아웃·표시 방식은 구현 재량이다.

**Agree All 선택 시**:
- consent를 Agree All로 기록한다.
- instruction 흐름을 정상 진행한다.

**Disagree All 선택 시**:
- "Disagree를 선택하면 이 테스트를 진행할 수 없습니다.
  그래도 Disagree하시겠습니까?" 계열의 짧은 안내와 최종 확인을 제공한다.
- 안내 문구의 정확한 표현은 구현 재량이다.
  계약 요건은 '테스트 불가 안내'와 '최종 확인 단계' 두 가지 제공이다.
...
- 게이트가 발동한 상태에서 consent 확인 없이 테스트 runtime에
  진입하면 안 된다.
- Disagree 최종 확인 이전까지는 consent 상태를 변경하면 안 된다.
- 게이트 표시 중 instruction overlay와의 상호작용 우선순위는
  구현 재량이나, 어느 쪽도 상대를 강제 닫으면 안 된다.

#### 관련 섹션 동기화 (§3.2 추가)

consent 게이트 발동 조건·동작 변경 시
§13.9, §13.10, req-test.md §1.2를 동일 변경셋으로 갱신한다.

**Verification**:
1. Automated: default 상태 + available 카드 진입 시 게이트가
   발동함을 검증한다.
2. Automated: Agree All 선택 후 테스트 runtime 진입이
   정상 허용됨을 검증한다.
3. Automated: Disagree 최종 확인 후 테스트 runtime 진입이
   차단되고 복귀 경로가 제공됨을 검증한다.
