# Landing Requirements — 동작 계약

**이 문서는 「무엇이 결과로 남는가」의 정본이다.** 짝 문서인 `docs/req-landing-interaction.md` 가 「어떻게 조작하고 어떻게 움직이는가」를 갖는다. 둘로 가른 이유는 이후의 인터랙션 개정이 동작 계약을 건드리지 않게 하기 위해서다(사용자 결정 6). 어느 쪽에 속하는지의 판정 기준과 두 문서의 우선순위는 그 문서 §0 이 갖는다 — 여기 재서술하지 않는다.
...
**두 문서 사이의 충돌**은 이 사슬 **위**에 있다 — `docs/req-landing-interaction.md` 는 이 문서의 불변식(§4)·라우팅(§5)·상태 결과(§7)·접근성 요건(§9)·telemetry(§12)를 override 하지 않는다. 같은 대상을 두 문서가 다르게 적으면 분할 실패이고, 고치는 방향은 인터랙션 쪽을 결과 기준으로 다시 쓰는 것이다.
...
- consent × instruction branch 정책 변경 시 (분기 조건, CTA 레이블, 저장 방식, 딥링크 케이스 분기 포함) Section §13.5, §13.9, §14.2, req-test.md §3.6을 동기화한다.
...
- `html lang`에 넣는 값은 제품 locale 코드가 아니라 **표시용 BCP 47 태그**다(`kr→ko` · `zs→zh-Hans` · `zt→zh-Hant`, 나머지 9개는 코드 그대로). 제품 코드 `kr`·`zs`·`zt`는 BCP 47이 아니므로 그대로 내보내면 WCAG 3.1.1을 어긴다. URL 세그먼트·스토리지 키·telemetry 필드의 정본은 제품 코드 그대로 유지하며, 바뀌는 것은 이 표시용 태그 하나뿐이다.
- BCP 47 또는 흔한 변형으로 들어온 locale 진입(`/ko`·`/zh-Hans`·`/en-US`·`/jp` 등)은 canonical 제품 코드 경로로 **redirect**한다. 별칭 판정은 앱 소유 경로 판정보다 **앞**에 와야 한다 — 그러지 않으면 별칭이 global unmatched 404로 떨어진다. canonical 세그먼트 자신은 별칭으로 취급하지 않는다(무한 redirect 방지).
...
- 전환 중 동일 카드가 Normal/Expanded로 동시에 보이면 안 된다. Expanded 카드가 다른 row 위를 시각적으로 덮는 것은 허용한다.
...
4. Automated: `needs_comp=false` 카드의 `comp_gap=0`과 추가 잉여 여백 `0`(전이 프레임 포함), 동일 row 자연 높이가 모두 같은 fixture에서 전 카드 `needs_comp=false` 및 `comp_gap=0`을 검증한다.
...
**Verification**:
1. Automated: 상태 전이 단위 테스트로 guard 조건을 검증한다. E2E에서 탭 전환 중 입력 차단을 검증한다.
2. Automated: ACTIVE 램프업 구간에서 Expanded/Collapse/Overlay 변형 시작 `0건`을 검증한다.
...
**Verification**:
1. Automated: 이벤트 순서 역전/지연 시나리오에서 동일 최종 상태를 검증하고, 재마운트 후 `scoring1`/runtime canonical index/user-facing scoring label 관계 역전 `0건`을 검증한다.
...
**Rule**: Desktop/Tablet 카드 키보드 탐색은 아래 순차 규칙을 최우선으로 따른다. Mobile lifecycle은 본 **순차 확장 override** 대상이 아니다 — 단 아래 7.6-a 의 전 표면 규칙은 Mobile 에도 적용된다.

#### 7.6-a Keyboard Affordance (전 표면 · 폭·입력 방식과 무관)
키보드는 뷰포트 폭에도 포인터 입력 방식에도 속하지 않는 **독립 축**이다. hover 가 없는 기기에 키보드가 없는 것이 아니며(외장 키보드를 붙인 태블릿·폰이 그것이다), 아래 넷은 모든 표면에서 성립해야 한다.
- 키보드로 도달 가능한 available Test 카드는 키보드로 확장·진입할 수 있어야 한다.
- 순회 순서는 결정적이어야 한다.
- unavailable 카드는 tab order 에서 제외된다.
- **확장된 카드는 `Escape` 로 닫을 수 있어야 한다.** 어느 생명주기(Desktop/Tablet overlay · Mobile lifecycle)로 열렸는지는 이 규칙을 면제하지 않으며, `Escape` 는 각 생명주기의 고유 닫기 경로로 라우팅된다.
...
- **탭 순서는 모든 컨텍스트에서 문서 순서를 따른다.** Landing이라고 해서 첫 `Tab`이 GNB를 건너뛰지 않으며, GNB control의 `tabIndex`를 상태에 따라 내리지 않는다. 종전 규칙(「첫 forward `Tab`이 첫 번째 available 카드로 진입」·「첫 카드에서 `Shift+Tab`이 마지막 visible GNB control로 복귀」)은 탭 순서를 상태의 함수로 만들어 예측 가능성을 해쳤고, 그 비용은 키보드 사용자에게만 부과됐다. 구현도 GNB DOM을 CSS 선택자로 뒤지는 계층 위반을 요구했다.
- 본문으로 바로 가려는 사용자를 위해 **skip link**를 둔다. 문서 순서상 첫 번째 tab stop이며, 포커스를 받을 때만 보이고, 목적지는 `<main>`이다(`tabindex="-1"`로 포커스를 받을 수 있어야 한다). 포커스를 잃으면 즉시 사라지고 레이아웃을 남기지 않는다. 라벨은 12 locale 메시지 키로 제공한다.
- 첫 카드에서의 `Shift+Tab`은 별도 처리 없이 브라우저의 문서 순서 이동에 맡긴다 — 확장된 카드가 있으면 닫되, 포커스 이동 자체를 가로채지 않는다.
...
**이 절은 인터랙션 계약으로 옮겼다 — `docs/req-landing-interaction.md` §8 이 정본이다.**

제스처와 그 판정, 모션의 시간·곡선·단계, 생명주기의 위상과 전이 연출, 닫기 어포던스의 종류와 우선순위가 그 문서로 갔다. 이 문서(동작 계약)에 남은 것은 그 상호작용이 **무엇을 결과로 남기는가**이며, 상태 집합과 전이의 결과는 §7 이, telemetry 는 §12 가, 접근성 요건은 §9 가 그대로 갖는다.
...
- 초기 렌더 경로에서 `window`, `localStorage`, `sessionStorage`, `Date.now`, `Math.random` 등 비결정 API 분기를 금지한다. `useState initializer`, `provider default`, `context init`에도 동일 적용한다.
...
### 11.2 Animation Guardrails · 11.3 Reduced Motion / Low-spec

**두 절은 인터랙션 계약으로 옮겼다 — `docs/req-landing-interaction.md` §11 이 정본이다.**
...
- landing preview는 항상 first scoring question(`scoring1`) 기준이다. profile 문항은 landing preview에 사용하지 않고 landing이 수집하는 입력은 `scoring1` preview 기반 A/B 선택뿐이다.
- landing A/B 선택값은 domain derivation token이 아니다. canonical index binding은 test runtime commit 시점의 책임이며, 세부 규칙은 `docs/req-test.md` §3.4/§3.5가 소유한다.
- 랜딩 fixture 최소: Test `4+`, Blog `3+`, unavailable Test `2+`. fixture 다양성 케이스 필수: 긴 텍스트, 빈 tags, debug fixture. required 슬롯 누락 금지. 런타임 adapter는 required 누락 시 throw하지 않고 normalize + default 삽입으로 방어해야 한다.
...
- 문항 체류시간은 포그라운드/백그라운드 상태와 무관하게 경과시간을 포함하며, 문항 재방문 시 누적 합산한다.
...
**Exception**: 전역 unmatched 404를 위해 `global-not-found` 파일 컨벤션과 버전 종속 설정을 함께 운용한다. **Risk**: Next.js 업그레이드 시 동작/설정 요구사항이 변동될 수 있다.

**Guardrails**: CI에서 Next 버전 고정 + 릴리스 전 404 E2E 회귀 테스트 필수. 현행 버전(`16.1.6`)은 `experimental.globalNotFound: true` 요구. 변경 시 근거/회귀 결과를 Change Log에 기록.
...
**Exception**: consent 상태가 `UNKNOWN`(미선택) 또는 `OPTED_OUT`인 경우, 비필수 클라이언트 텔레메트리의 네트워크 전송을 금지한다. 사용자가 동의 여부를 선택하지 않았거나 거부한 상태에서 텔레메트리가 전송되는 리스크를 차단한다. 텍스트 이벤트 전송 금지에 한정하며, 카드 가시성 필터링(§13.9)은 별도 계약이다.
