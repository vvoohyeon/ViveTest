...
| # | ADR 주제 | 선택지 | 결정 상태 |
|---|---|---|---|
| ADR-C | Google Sheets API 인증 방식 | Service Account vs OAuth (GitHub Action 시크릿 `GOOGLE_SHEETS_ID_LANDING` / `GOOGLE_SHEETS_ID_QUESTIONS` / `GOOGLE_SHEETS_ID_RESULTS` 구성 포함) | ⬜ 미결 |
...
| 산출물 | 내용 |
|---|---|
| `VariantRegistry` 타입 | `variant-registry.generated.ts`의 runtime/export 스키마. `landingCards`와 `testPreviewPayloadByVariant`를 분리 보관하고, source 전용 필드(`seq`, raw preview source)는 exported card payload에 남기지 않는다. generated registry는 code-owned canonical mapping에서 resolve된 schema를 포함한다. 상세 타입 계약: `docs/req-test.md` §2.5 |
...
| Questions parser | Questions workbook에서 `sheet name = variantId`를 적용하고, `seq`에서 `questionType`을 생성하며, source row 순서를 유지한 canonical index 재번호를 수행 |
...
| preview derivation | landing preview payload는 Questions source의 `scoring1`에서 파생한다. profile row는 preview source가 아니다 |
