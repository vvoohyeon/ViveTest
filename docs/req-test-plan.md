...
| EGTT dev/test fixture | `axisCount=1`, `qualifierFields` 1개 구조의 dev fixture. Phase 1 EGTT schema 지원 검증 및 Phase 4 이후 진입 경로 테스트에 사용. Questions row fixture는 `src/features/test/fixtures/questions/egtt.ts`에서 profile row(`seq=q.1`)와 scoring rows를 보존한다. Landing/card fixture source는 3-source topology + code-owned schema registry 결합을 builder를 통해 `variant-registry.generated.ts` runtime/export 인터페이스로 변환해야 한다 |
