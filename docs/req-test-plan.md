...
- 현재 dev fixture는 `src/features/test/fixtures/questions/**` 아래 variant별 파일로 sheet-name 분리를 모사한다.
- Sheets의 snake_case locale 컬럼(`question_EN`, `answerA_EN` 등)은 fixture에서 `LocalizedText` 기반 `question`, `answerA`, `answerB` object로 정규화한다.
...
### scoringLogicType canonical location 결정

- `scoringLogicType` (ScoringLogicType: 'mbti' | 'egtt')의 canonical location은
  `src/features/test/schema-registry.ts`로 확정한다.
- 이 정보는 계산 알고리즘 선택을 위한 코드 계약이며,
  Landing metadata, Questions fixture, Sheets 컬럼에 중복 선언하지 않는다.
- 외부 조회는 `getLogicTypeForVariant()` / `getSchemaForVariant()` API를 통해서만 수행한다.
- 향후 logic type 추가 시 ScoringLogicType union, schemaTemplates, variantLogicTypeMap을
  동일 변경셋에서 확장하고 schema-registry.test.ts의 registeredVariants 목록도 함께 갱신한다.
...
// schema axis와 역방향인 question도 같은 axis로 집계한다
...
| `schema-registry.ts` | code-owned canonical schema registry. 현재 `ScoringLogicType = 'mbti' | 'egtt'`이며, `logicType -> schema template`, `variant -> logicType` 매핑으로 `getSchemaForVariant(variantId)`를 제공한다. `scoringLogicType` canonical location은 `src/features/test/schema-registry.ts`이며 Landing metadata, Questions fixture, Sheets 컬럼에 중복 선언하지 않는다. EGTT schema는 `axisCount=1`, axis `E/T`, qualifier `gender`, values `['M', 'F']`, `tokenLength=1`로 고정한다. runtime registry 연결은 후속 범위다 |
| `response-projection.ts` | Phase 4/7 projection layer 예약 파일. runtime `'A' | 'B'` 응답을 domain token으로 변환하는 pure helper 위치이며, 이번 단계에서는 함수 구현 없이 계약 주석만 둔다. `computeScoreStats()` / `buildTypeSegment()`가 raw A/B를 직접 소비하지 않는 경계를 보존한다 |
| `loadVariantRegistry()` | 환경별 registry 파일 로딩 인터페이스. production과 dev fixture가 동일 시그니처. 현재 구현은 ADR-D 미결 상태를 반영해 항상 cached fixture-built registry를 반환하며, generated production 분기는 ADR-D 확정 후 연결한다 |
...
| Questions parser / fixture boundary | Questions workbook에서 `sheet name = variantId`를 적용하고, `seq`에서 `questionType`을 생성하며, source row 순서를 유지한 canonical index 재번호를 수행해야 한다. 현재 dev fixture 경계는 `src/features/test/fixtures/questions/index.ts`이며, variant별 파일은 Sheets row를 `LocalizedText` 기반 row shape로 보존한다. `src/features/test/question-source-parser.ts`는 `parseSeqToQuestionType()`, `buildCanonicalQuestions()`, `findFirstScoringRow()`를 제공한다. 이후 preview source를 Questions의 **first scoring question** 으로 교체할 수 있도록 parser/builder 경계를 설계한다 |
| `validateCrossSheetIntegrity(landingTestVariants, questionVariants)` | Landing test variant set과 Questions workbook sheet-name set의 2-source 정합성을 검증하는 pure 함수. `missingInQuestions` / `extraInQuestions`를 반환한다. Results source까지 포함한 3-source 검증, GitHub Action 스크립트 연결, runtime 2차 방어선 연결은 후속 구현 범위다 |
...
| preview derivation | 현재 단계에서는 landing preview payload source를 fixture inline **temporary bridge** 로 유지할 수 있어야 한다. 이 bridge는 Landing registry source fixture에만 남기고 resolver boundary로만 주입해야 하며, Questions source의 `scoring1`은 다음 단계 migration target으로 문서화한다. profile row는 preview source가 아니다. `resolveVariantPreviewQ1()`는 Questions fixture 기반 `scoring1` 후보 selector로 구현되었지만 live landing preview wiring은 아직 `resolveTestPreviewPayload()`를 사용한다 |
...
| EGTT dev/test fixture | `axisCount=1`, `qualifierFields` 1개 구조의 dev fixture. Phase 1 EGTT schema 지원 검증 및 Phase 4 이후 진입 경로 테스트에 사용. 현재 code-owned schema registry 기준 token은 대문자 `E/T`, `M/F`다. Questions row fixture는 `src/features/test/fixtures/questions/egtt.ts`에서 profile row(`seq=q.1`)와 bidirectional scoring rows(`E/T`, `T/E`)를 보존한다. 현재 scoring rows 2개는 같은 axis의 even-count fixture로 간주되며, schema registry와 `validateVariantDataIntegrity()` 연결은 후속 범위다. Landing/card fixture source는 3-source topology + code-owned schema registry 결합을 builder를 통해 `variant-registry.generated.ts` runtime/export 인터페이스로 변환해야 한다 |

### Phase 2 DoD (Policy Additions)

- source fixture와 runtime/export 타입을 분리해야 한다. Landing registry source에만 `seq`와 temporary inline preview bridge를 허용하고, runtime landing card payload에는 이를 노출하지 않는다. Questions source row는 `seq`와 `LocalizedText` 기반 question/answer content를 보존하되 runtime question model의 canonical index와 분리한다.
- preview payload는 fixture inline bridge를 유지하더라도 Landing registry source fixture에만 남겨야 한다. landing / test / blog consumer의 raw fixture access는 금지한다.
- 배열 순서 계약 `seq -> sort -> drop`을 구현해야 한다. missing / duplicate / invalid `seq`는 fixture validation fail로 처리해야 하며, consumer는 배열 순서만 신뢰하게 만든다. Questions parser도 source row 출현 순서를 기준으로 canonical index를 재번호해야 한다.
- 현재 구현된 Questions parser / bank selector 단위 테스트 앵커는 `tests/unit/question-source-parser.test.ts`와 `tests/unit/variant-question-bank.test.ts`다. 기존 `tests/unit/landing-question-bank.test.ts`는 inline bridge 기반 하위 호환 경로를 계속 검증한다.
...
- [ ] `validateCrossSheetIntegrity()`를 Results source 포함 3-source 검증과 Sync/runtime 호출부에 연결 완료 여부 확인
