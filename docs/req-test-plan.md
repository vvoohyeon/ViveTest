...
| 3-source loader | `GOOGLE_SHEETS_ID_LANDING/QUESTIONS/RESULTS` 각각 로드. Questions workbook: sheet name = variantId. |
...
| GitHub Action 스크립트 골격 | 3개 source 읽기 → code-owned schema resolution → 3-source 검증 → 성공 시 registry 파일 생성 → 커밋 트리거. |
| Generated registry serializer | `src/features/variant-registry/registry-serializer.ts`: `VariantRegistry`를 object-literal 기반 `variant-registry.generated.ts` 파일 문자열로 직렬화한다. 기존 generated header와 typed export를 유지하고 plain object key를 알파벳 순으로 정렬해 재생성 diff noise를 줄인다. 파일 쓰기와 full Sheets sync wiring은 Group B 소유로 남는다. |
| Fixture-only generated regeneration | `scripts/sync/regenerate-variant-registry-from-fixture.ts`가 현재 fixture + builder 출력으로 `variant-registry.generated.ts` static object literal을 재생성한다. Full Google Sheets Sync 자동화는 여전히 Group B 소유. |
...
- [ ] GitHub Action 스크립트가 동일 3-source helper를 pre-commit 차단 함수로 호출하는지 확인
