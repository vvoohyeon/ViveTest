...
**완료 조건**:
- [ ] `theme-matrix-manifest.json` 대표 케이스 범위 재정의 (현재 168장 → 최소화 기준 확정)
- [ ] 저장소 추적 대상 baseline PNG 목록 확정 (현재 36장 추적, 132장 `.gitignore` 상태 해소 방향 결정)
- [ ] Safari ghosting baseline 디렉토리 복구 범위 결정
- [ ] test flow 신규 settle recipe 케이스 추가 시 manifest + baseline 추가 범위 결정 기준 문서화
...
다음 ADR 모두 완료된 경우에만 Phase 1 착수를 허용한다.

| ADR | 완료 여부 |
|---|---|
| ADR-A: 네임스페이스 분리 + clean-room ADR | ⬜ 미완료 |
| ADR-B: Storage Key + 5개 플래그 계약 | ⬜ 미완료 |
| ADR-E: Representative variant + manifest-baseline 축소 범위 확정 | ⬜ 미완료 |
...
Phase 3 완료 후, Phase 4(Entry Path · Staged Entry · Invalid Variant Recovery) 착수 전에 아래 두 항목을 확인한다.

**① Staged Entry Expiry 경계 계약 확인**
...
**② Cross-phase Event Integrity 픽스처 소유권 확인**
...
### Content Fallback 동작 확인 (AR-004 기준)
...
### `adapter.ts` tolerant normalization 교체 경로 확인
...
## Part 6 — Phase 11 착수 전 확인 (Telemetry Skeleton · Release Gate)
...
**ADR-G — blocker-traceability.mjs 상한 확장**

- [ ] `check-blocker-traceability.mjs` 현재 blocker 1~19 하드코딩을 1~30으로 확장 완료
- [ ] 확장 타이밍: Phase 11 착수 시점. 담당자 결정 및 기록 완료
- 미완료 시: Phase 11 traceability closure(blocker 1~30 매핑) 실행 불가. Phase 11 gate 영구 FAIL.

**ADR-H — test-flow telemetry hook 검사 추가**
