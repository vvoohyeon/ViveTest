...
| `final_submit` | same as above + `final_responses` (canonical question index string keys: `"1"`, `"2"`, ...; values: non-empty strings; scoring answers use semantic `A`/`B`, qualifier answers use schema tokens such as `M`/`F`) |
