...
**Test** (`src/features/test/test-question-client.tsx`): policy-driven instruction gating, variant Questions fixture runtime wiring through `buildVariantQuestionBank()`, landing-ingress seeding of `scoring1`, direct-entry start at canonical question 1, dwell time tracking, and `attempt_start` / `final_submit` telemetry. Ingress entries keep the landing `scoring1` pre-answer, then start at the first current-fixture question that is not the first scoring question: `qmbti` starts at canonical index 2, while profile-first `egtt` starts at canonical index 1 (`q.1`). The live question panel uses all resolved fixture rows, renders a `Q{canonicalIndex}` label above the question text, displays main progress as answered scoring count / total scoring count with the percent label inside the fill bar, and uses a 150 ms delayed auto-advance as the forward navigation mechanism for non-last questions; the explicit Next button has been removed. Previous remains present in the DOM and is hidden at Q1 with `visibility: hidden` so navigation space is preserved. The answer grid uses `motion/react` directional slide transitions on question navigation, with reduced-motion removing horizontal movement. The `started` flag is retained as the active-run UI guard signal, and a `beforeunload` listener is active only during a started, not-yet-submitted run before being removed on submit and unmount. It does not yet run score derivation, result URL construction, answer projection into domain tokens, active-run resume, or history persistence. `writeResponseSet` is called on every answer update via `use-test-run-controller.ts`. The resume read path (`readResponseSet`, `getActiveRun`) is intentionally deferred to Phase 4/5 — the write-without-read asymmetry is not an oversight. `final_submit.final_responses` uses canonical question index string keys (`"1"`, `"2"`, ...) mapped to semantic `A` / `B` codes. `src/features/test/entry-policy.ts` separates content, CTA configuration, and action effects; `src/features/test/instruction-overlay.tsx` renders the composed instruction surface. `src/app/[locale]/test/[variant]/page.tsx` regex-validates the URL segment, redirects runtime-blocked variants to `/test/error?variant=...`, resolves via `resolveLandingTestEntryCardByVariant(locale, variant)`, fails closed with `notFound()` on true miss, and runs `getLazyValidatedVariant()` before mounting the runtime.
