...
**Test** (`src/features/test/test-question-client.tsx`): policy-driven instruction gating, variant Questions fixture runtime wiring through `buildVariantQuestionBank()`, landing-ingress seeding of `scoring1`, direct-entry start at canonical question 1, dwell time tracking, and `attempt_start` / `final_submit` telemetry. Ingress entries keep the landing `scoring1` pre-answer, then start at the first current-fixture question that is not the first scoring question: `qmbti` starts at canonical index 2, while profile-first `egtt` starts at canonical index 1 (`q.1`). `src/features/test/test-run-reducer.ts` now owns the booting / instruction / active / submitted / redirecting phase graph, canonical-index keyed answers, tail reset, submit guards, and entry-mode signals for new vs resume entry. `use-test-run-controller.ts` owns the reducer instance, reads `getActiveRun()` + `readResponseSet()` only when no Landing Ingress exists, writes active-run metadata on new runtime entry, initializes a fresh response set on every new entry, refreshes `lastAnsweredAtMs` on confirmed answers, and preserves Landing Ingress priority over older response sets. Active-Run Resume restores filtered canonical answers, routes missing profile prerequisites back through the instruction/profile path by clearing untrustworthy `instructionSeen`, then resumes at the first unanswered required canonical question capped at the final question; it does not re-emit `attempt_start`. The live question panel uses all resolved fixture rows, renders user-facing `Q` labels only for scoring questions using scoring order, displays main progress as answered scoring count / total scoring count with the percent label inside the fill bar, and uses a 150 ms delayed auto-advance as the forward navigation mechanism for non-last questions; the delayed advance captures the clicked choice before React render catch-up and skips already-answered resume gaps. Previous remains present in the DOM and is hidden at Q1 with `visibility: hidden` so navigation space is preserved. The answer grid uses `motion/react` directional slide transitions on question navigation, with reduced-motion removing horizontal movement. The `started` flag is retained as the active-run UI guard signal, and a `beforeunload` listener is active only during a started, not-yet-submitted run before being removed on submit and unmount. It does not yet run score derivation, result URL construction, answer projection into domain tokens, result-entry cleanup, or history persistence. `final_submit.final_responses` uses canonical question index string keys (`"1"`, `"2"`, ...) mapped to semantic `A` / `B` codes. `src/features/test/entry-policy.ts` separates content, CTA configuration, and action effects; `src/features/test/instruction-overlay.tsx` renders the composed instruction surface. `src/app/[locale]/test/[variant]/page.tsx` regex-validates the URL segment, redirects runtime-blocked variants to `/test/error?variant=...`, resolves via `resolveLandingTestEntryCardByVariant(locale, variant)`, fails closed with `notFound()` on true miss, and runs `getLazyValidatedVariant()` before mounting the runtime.
...
- `vivetest-test-instruction-seen:{variant}` is still variant-scoped in `sessionStorage`; active-run resume clears it when stored resume data is inconsistent with required profile/qualifier prerequisites.
