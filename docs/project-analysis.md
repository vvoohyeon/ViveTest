...
|---|---|
| Test destination | Policy-driven instruction + qualifier overlay/re-entry keyed by ingress type + consent state + `attribute`, registry-backed instruction copy, reducer-backed scoring question flow, answer lock + delayed auto-advance, manual previous tail reset, active-run resume, dwell tracking, placeholder result panel, `attempt_start` / `question_answered` / `final_submit` / temporary `result_viewed` telemetry |
...
| `src/features/test/` | Canonical test destination surface — route/runtime shell, instruction entry policy, reducer/bootstrap/controller hooks, overlay/result composition, question-bank resolution, and test storage |
...
**Test** (`src/features/test/test-question-client.tsx` and collaborators): the client is now a composition/render shell rather than the owner of the whole runtime. `test-run-reducer.ts` owns the booting / instruction / active / submitted / redirecting phase graph, canonical-index keyed string answers, qualifier answer merging, scoring-answer reset, previous-navigation tail reset, submit guard, and new-vs-resume entry mode. `use-test-run-bootstrap.ts` owns pending-transition hydration, Landing Ingress priority, active-run + response-set reads, qualifier resume validation, stale qualifier cleanup, and `BOOTSTRAP_COMPLETE` dispatch. `use-test-run-controller.ts` owns the reducer instance, scoring progress, response persistence, active-run metadata writes, dwell tracking through `use-question-dwell.ts`, and `attempt_start` / `final_submit` telemetry. `use-test-entry-orchestrator.ts` interprets instruction CTA effects and composes `use-qualifier-overlay-wizard.ts`, `use-entry-side-effects.ts`, and `use-auto-commit.ts`; entry side effects write consent, mark `instructionSeen`, and clear Landing Ingress on redirect. `OverlayConnector` / `InstructionOverlay`, `QualifierChip`, and `ResultConnector` / `TestResultPanel` keep presentation mapping separate from reducer/runtime ownership.

Ingress entries keep the landing `scoring1` pre-answer in reducer answers at commit, then start the runtime panel at the first scoring question that still needs runtime display: `qmbti` starts at canonical index 2, and `egtt` now also starts at the first scoring question (canonical index 2) because `q.1` is a qualifier question collected in the instruction overlay before runtime entry. Active-Run Resume restores filtered canonical answers when qualifier prerequisites are valid; missing or invalid qualifier answers clear the stale run context and return through instruction/qualifier collection, while valid resume skips `attempt_start`. The live question panel renders scoring questions only, renders user-facing `Q` labels using scoring order, displays main progress as answered scoring count / total scoring count with the percent label inside the fill bar, and uses `use-answer-lock.ts` for the 150 ms delayed auto-advance on non-last scoring questions. The delayed advance captures the clicked choice before React render catch-up, disables answer buttons during the wait, and skips already-answered resume gaps. Previous remains present in the DOM and is hidden at Q1 with `visibility: hidden` so navigation space is preserved. Qualifier variants expose a chip after entry; re-entry seeds the existing qualifier token, cancel preserves answers, and confirm writes qualifier-only responses before restarting at the first scoring question. The answer grid uses `motion/react` directional slide transitions on question navigation, with reduced-motion removing horizontal movement. `use-before-unload-guard.ts` registers the native reload/close prompt only during a started, not-yet-submitted run, and `use-landing-transition-completion.ts` completes the destination-side pending transition once the runtime is ready. It does not yet run score derivation, result URL construction, answer projection into domain tokens, result-entry cleanup, or history persistence. `final_submit.final_responses` uses scoring canonical question index string keys (`"1"`, `"2"`, ...) mapped to semantic `A` / `B` values and excludes qualifier/profile tokens. `question_answered` fires from the answer choice call site for non-last scoring answers, and the placeholder result panel fires `result_viewed` once on mount without `derived_type` until the real result pipeline replaces it with IntersectionObserver. `src/features/test/entry-policy.ts` separates content, CTA configuration, and action effects; `src/features/test/qualifier-overlay-model.ts` maps qualifier fields to overlay items; `src/features/test/qualifier-resume-validator.ts` guards resume against missing or invalid qualifier tokens. `src/app/[locale]/test/[variant]/page.tsx` regex-validates the URL segment, redirects runtime-blocked variants to `/test/error?variant=...`, resolves via `resolveLandingTestEntryCardByVariant(locale, variant)`, fails closed with `notFound()` on true miss, and runs `getLazyValidatedVariant()` before mounting the runtime.
...
**Least verified areas** correspond directly to unimplemented or stubbed product surfaces: live score derivation wiring, contracted result screen rendering, test error recovery cards, answer projection from runtime `A`/`B` to domain tokens, real `derived_type` + IntersectionObserver-backed `result_viewed`, user-visible error telemetry, history persistence, backend ingestion guarantees, Results Sheets loading, and branch-protected production push policy.
...
`src/app/[locale]/test/[variant]/page.tsx` · `src/app/[locale]/test/error/page.tsx` · `src/features/test/test-question-client.tsx` · `src/features/test/test-run-reducer.ts` · `src/features/test/use-test-run-controller.ts` · `src/features/test/use-test-run-bootstrap.ts` · `src/features/test/use-test-entry-orchestrator.ts` · `src/features/test/use-qualifier-overlay-wizard.ts` · `src/features/test/use-entry-side-effects.ts` · `src/features/test/use-auto-commit.ts` · `src/features/test/use-answer-lock.ts` · `src/features/test/use-question-dwell.ts` · `src/features/test/use-before-unload-guard.ts` · `src/features/test/use-landing-transition-completion.ts` · `src/features/test/entry-policy.ts` · `src/features/test/overlay-connector.tsx` · `src/features/test/instruction-overlay.tsx` · `src/features/test/qualifier-chip.tsx` · `src/features/test/result-connector.tsx` · `src/features/test/test-result-panel.tsx` · `src/features/test/qualifier-overlay-model.ts` · `src/features/test/qualifier-resume-validator.ts` · `src/features/test/question-runtime-utils.ts` · `src/features/test/question-bank.ts` · `src/features/test/question-source-parser.ts` · `src/features/test/lazy-validation.ts` · `src/features/test/canonical-key.ts` · `tests/unit/question-source-parser.test.ts` · `tests/unit/variant-question-bank.test.ts` · `tests/unit/test-question-bootstrap.test.ts` · `tests/unit/test-entry-policy.test.ts` · `tests/unit/test-entry-orchestrator-qualifier.test.ts` · `tests/unit/test-entry-orchestrator-reentry.test.ts` · `tests/unit/use-test-entry-orchestrator.test.ts` · `tests/unit/use-test-run-controller.test.ts` · `tests/unit/test-run-reducer.test.ts` · `tests/unit/overlay-connector.test.ts` · `tests/unit/qualifier-chip.test.ts` · `tests/unit/test-result-panel.test.ts` · `tests/unit/qualifier-overlay-model.test.ts` · `tests/unit/qualifier-resume-validator.test.ts` · `tests/unit/test-lazy-validation.test.ts` · `tests/e2e/qualifier-overlay.spec.ts` · `docs/req-test.md` · `docs/req-test-plan.md`
...
**Test entry orchestration is now split from the client.** `use-test-entry-orchestrator.ts` owns the entry action handler (`executeInstructionAction`) and composes the qualifier wizard, entry side effects, and auto-commit scheduling. `use-qualifier-overlay-wizard.ts` owns qualifier step/re-entry draft state; `use-entry-side-effects.ts` owns consent writes, `markInstructionSeen`, and Landing Ingress clearing; `use-auto-commit.ts` owns instruction-seen auto-entry for non-qualifier variants. `test-question-client.tsx` retains `instructionVisible` derivation and renders connector props. The orchestrator path does not own telemetry — `trackAttemptStart` and `trackFinalSubmit` remain in `use-test-run-controller.ts`, while `question_answered` fires from the answer choice call site.

**Test question auto-advance is still presentation-owned, but no longer inline.** `use-answer-lock.ts` owns the 150 ms delayed auto-advance timer and answer-button lock state. Rapid Previous taps clear the timer before backward navigation fires, and a client-local `submittedRef` mirror prevents the delayed timer from advancing after manual Submit on the last question.
