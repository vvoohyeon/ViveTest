...
| Scoring schema | schema-driven, axisCount ∈ {1, 2, 4} 가변. MBTI 4축 하드코딩 금지 |
...
| Result URL 인코딩 | `variant`은 path segment 1, `type`(derivedType 토큰)은 path segment 2. `scoreStats`와 `shared`(boolean)는 JSON → URL-safe Base64 → 키 없는 query string. `scoringSchemaId`는 URL 어느 위치에도 포함하지 않는다. `variant`가 스키마의 유일한 식별자 |
...
- **Result-entry Eligibility**: 마지막 문항 응답이 유효하게 선택되고, `all-required-answered = true`이며, 결과 진입을 막는 blocking 상태가 없을 때 성립하는 자격 상태. 위치 기반이 아니라 논리 조건 기반이다. 상세 계약은 §2.10 참조.
...
- variant 검증 실패 시 runtime 진입을 차단하고, 에러 복구 페이지로 이동한다 (§5.1 참조).
...
**불변식**:
- Landing Ingress + active run 존재 시 반드시 §3.2의 프롬프트를 표시해야 한다.
...
- commit failure: §5.6 Commit-failure Error State로 처리.
...
**일반 계약**:
- 모든 runtime 진입은 §2.3의 세 경로 중 정확히 하나로 분류되어야 한다.
- 진입 경로가 확정되기 전에 instruction, question runtime, result 중 어느 단계도 시작하면 안 된다.
- variant 유효성 검증은 진입 경로 분류 이전에 완료되어야 한다.
- variant 검증에 실패한 경우 §2.3의 어느 경로로도 분류되지 않으며, §5.1 에러 복구 페이지로 즉시 이동한다.
- 진입 경로와 variant 문맥은 해당 run이 terminal 상태에 도달할 때까지 일관되게 유지되어야 한다.
- 진입 경로가 바뀌는 경우(예: Landing Ingress → 처음부터 다시 하기)는 §7.3 cleanup set을 완료한 뒤 새 경로로 재분류한다.

### 2.5 Staged Entry Contract
...
### 2.7 Session / Run Lifecycle Contract

- session identity는 instruction, question runtime, result 전 구간에서 연속성을 유지해야 한다.
- 재사용 가능한 active run은 탐지 가능해야 한다 (§2.1 Active Run 정의 기준).
...
- 30분 초과 시 → timeout 처리. §5.8 휘발 계약 즉시 적용 후 Cold Start.
...
### 2.8 Question Model Contract

- 각 question은 정확히 2개의 선택지를 가져야 한다. 두 선택지의 의미는 `poleA`, `poleB` 문자열로 직접 표현한다.
- 각 question은 정확히 1개의 axis를 평가해야 한다. 축 소속은 별도 `axisId` 필드가 아니라, question의 `poleA`+`poleB` 쌍이 해당 variant의 `schema.axes` 중 동일 쌍과 일치하는 항목으로 결정된다.
- 하나의 question이 다중 axis에 동시에 기여하면 안 된다. poleA+poleB 쌍이 schema에서 둘 이상의 axis에 매칭되어서는 안 된다.
...
### 2.9 Progress / Revision / Tail Reset Contract
...
- tail reset 즉시: 이전 derivation attempt residue 전체를 폐기한다 (§7.3 cleanup 참조).
- **마지막 문항의 응답 변경은 tail reset을 발생시키지 않는다.** 이전 derivation residue만 무효화하며, 마지막 문항의 새 응답이 유효하면 result-entry eligibility는 유지된다 (§2.10 참조).

### 2.10 Result-entry Eligibility Contract
...
### 2.11 Derivation Model Contract

- 각 released variant는 다음을 선언해야 한다.
  - `axisCount` (`1`, `2`, `4`)
  - `axes`: ordered `AxisSpec[]`. 각 항목은 `{ poleA: string, poleB: string }`. 길이는 `axisCount`와 일치해야 한다.
  - `scoringSchemaId`
...
- 각 axis는 홀수 question count를 가져야 한다. 위반 시 blocking validation error로 처리한다.
...
AxisScoreStat = {
  ...
  dominant: string                   // counts 기준 다수 pole. 홀수 문항 전제로 동점 불가
}
...
**도출 절차**:
1. `schema.axes`를 선언 순서대로 순회한다.
2. 각 `AxisSpec { poleA, poleB }`에 대해 `axisId = poleA + poleB`를 파생한다.
3. 해당 axisId에 속하는 문항(poleA+poleB 쌍 일치 기준)의 응답을 집계해 `counts`를 구성한다.
4. `counts` 기준 더 높은 pole을 `dominant`로 결정한다. 홀수 문항 보장 하에 동점은 발생하지 않는다.
5. `scoreStats[axisId] = { poleA, poleB, counts, dominant }`를 기록한다.
6. `derivedType` 토큰은 `schema.axes` 순서대로 각 axis의 `dominant`를 연결해 생성한다.

**Tie 처리 정책**:
- 정상 경로(odd-count rule 준수 데이터)에서 동점은 발생하지 않는다.
- odd-count rule 위반은 §5.2 blocking data error로 사전 차단된다. 런타임 tiebreak 로직은 주 경로에 포함하지 않는다.

### 2.12 Result Rendering Contract

result 페이지 렌더링의 추상 원칙 계약이다.  
payload 구조 계약(§4.1)과 분리된 레이어로, §5.4와 §6.1은 이 계약을 위임받아 세부 구현 기준으로 사용한다.
...
### 3.2 Landing Ingress + Active Run 프롬프트
...
- commit failure 시: §5.6 Commit-failure Error State로 처리.

**[처음부터 다시 하기]**:
- commit success 시: 기존 응답 데이터 즉시 완전 삭제 → Q1 pre-answer를 새 run의 Q1 응답으로 저장 → Q2부터 새 run 시작. Instruction 표시 여부는 §2.6 `instructionSeen` 계약 준수.
- commit failure 시: §5.6 Commit-failure Error State로 처리.
...
- 이전 문항으로 이동해 응답을 변경하면 tail reset이 발생한다 (§2.9 참조).
- 마지막 문항의 응답을 변경하면 이전 derivation residue가 무효화된다. tail reset은 발생하지 않는다 (§2.9 참조).
...
**마지막 문항 응답 변경 시**:
- 이전 derivation attempt residue는 변경 확정 즉시 무효화된다 (§7.3 derivation residue cleanup 범위).
- 마지막 문항의 새 응답은 유지된다.
- `all-required-answered = true`는 유지된다.
- result-entry eligibility는 논리 조건에 따라 즉시 재평가하며, 조건이 충족되면 유지된다.

### 3.5 Completion → Result-entry Eligibility

- 마지막 required question까지 응답이 완료되면 run은 result-entry eligibility를 획득한다 (§2.10).
...
**응답 데이터 삭제 시점**: result screen entry commit 완료 직후 §5.8 휘발 계약을 실행한다. 계산 완료(`derivation_computed = true`)만으로는 응답 데이터를 삭제하지 않는다.
...
**Derivation failure**: 계산 자체가 실패하면 §5.7 Derivation-failure Error State로 처리한다.

### 3.7 Back-from-Loading
...
## 4. Result URL & 케이스 분기 계약

### 4.1 Self-contained Payload 구조
...
- `type`: 최종 계산된 derivedType 토큰. URL path segment 2. 토큰 길이는 해당 variant의 `axisCount`와 일치해야 한다.
- `base64Payload`: `scoreStats`와 `shared`(boolean)만 포함한다. JSON 직렬화 후 URL-safe Base64로 인코딩해 **키 없는 query string** 위치에 단독으로 추가한다.
- 공유 여부는 `shared` 필드를 base64 payload 내부에 boolean으로 포함한다. 별도 named query parameter(`share=y` 등)를 사용하지 않는다.
...
- `scoreStats` key는 `poleA+poleB` 파생 문자열 (`axisId`). schema 선언 순서로 직렬화한다.
- `counts`는 pole 문자열을 key로 하는 응답 수 Record다.
- axisCount=1 variant는 `scoreStats` entry가 1개, axisCount=2는 2개다. 구조는 동일하다.

**Base64 인코딩 규칙**: URL-safe Base64 (`+`→`-`, `/`→`_`, padding `=` 제거).

**불변식**:
- payload는 untrusted input으로 취급하며, 수신 시 Base64 디코딩 → JSON 파싱 → schema validation 순서로 검증을 강제한다.
- payload에 원문 질문/답변 텍스트, PII를 포함하면 안 된다.
- `variant` path segment으로 schema를 조회할 수 없는 경우 §5.3 invalid payload로 처리한다.
- `type` path segment 길이는 `variant`로 식별된 schema의 `axisCount`와 일치해야 한다. 불일치 시 §5.3 invalid payload로 처리한다.
- `scoreStats` 구조는 `variant`로 식별된 schema 선언과 일치해야 한다. 불일치 시 §5.3 invalid payload로 처리한다.
- 공유 URL 생성 시 payload의 `shared` 필드를 `true`로 설정한다 (이번 단계 구현 제외, 구조만 예약).
...
## 5. Error / Fallback / Rollback / Restoration Contract

### 5.1 Invalid Variant Input
...
3. 완료 여부 판단 기준: local storage (이번 Phase). history 구현 이후에는 local storage + history 모두 적용 (§8.2 의무 항목 참조).
...
- JSON 파싱 실패
- 필수 필드 누락 (`scoreStats`)
- `type` 토큰 길이가 `variant`로 식별된 schema의 `axisCount`와 불일치
- `scoreStats` 구조가 `variant`로 식별된 schema 선언과 불일치
...
4. Automated: `type` 토큰 길이와 schema `axisCount` 불일치에서 에러 렌더링을 검증한다.
5. Automated: `variant`에 해당하는 schema 조회 실패에서 에러 렌더링을 검증한다.

### 5.4 Result Content Fallback

§2.12 Result Rendering Contract에서 위임받는 세부 구현 기준이다.
...
| Commit-failure | Blocking | §5.6 처리: old run 보존 + 복구 액션 |
| Derivation-failure | Blocking | §5.7 처리: 재시도 또는 마지막 문항 복귀 |
| Telemetry hook 실패 | Non-blocking | 핵심 flow 계속 진행 |

### 5.6 Commit-failure Error State
...
> error state에 머무는 동안에도 old active run inactivity timeout은 freeze되지 않는다 (§2.7 참조).

### 5.7 Derivation-failure Error State
...
**기본 허용 액션**:
- **마지막 문항으로 돌아가기**: §3.7 Back-from-Loading 계약을 따른다.
...
1. `derived_type` — derivedType 토큰 강조 표시 (axisCount에 따른 토큰 길이)
2. `axis_chart` — Axis score 시각화 (scoreStats 기반, schema 선언 순서. axisCount 1/2/4 모두 지원)
...
## 7. Terminal Exclusivity & Cleanup Set

### 7.1 Runtime Terminal Exclusivity
...
### 7.2 State Hygiene / Storage Contract
...
| Derivation-failure 발생 | §5.7 폐기 항목 목록 전체. 응답 집합 + eligibility 유지 |
| 마지막 문항 응답 변경 | §5.7 폐기 항목 목록과 동일 범위의 derivation residue 무효화. 응답 집합 + eligibility 유지 가능 |
| 이전 문항 응답 변경 (tail reset) | 변경 이후 응답 전체 + §5.7 폐기 항목 목록과 동일 범위의 derivation residue 전체 폐기 |
...
- **기존 계약 영향**: §2.2의 "fallback variant 사용" 계약을 "에러 복구 페이지 이동" 계약으로 대체함. "deterministic fallback variant" 개념은 이 문서에서 제거됨.
- **Affected Sections**: 2.2, 2.4, 5.1, 11.2(#1)

### AR-002 Inactive Session Timeout Duration
- **Status**: ✅ 확정 (30분, 재진입 시점 판정)
- **Affected Sections**: 2.7, 5.8, 7.3
...
- **Affected Sections**: 2.12, 5.4, 6.1
...
## 10. Single-change Synchronization

정책이 변경될 때 함께 갱신해야 하는 섹션 묶음을 명시한다.

| 변경 트리거 | 동기화 대상 섹션 |
|---|---|
| Variant resolution / 에러 복구 카드 정책 변경 | 2.2, 2.4, 5.1, 7.1, 11.2 |
| Session / active run / timeout 정책 변경 | 2.6, 2.7, 3.2, 5.8, 5.9, 7.1, 7.3, 11.2 |
| instructionSeen 기록·리셋 정책 변경 | 2.6, 5.8, 7.3, 11.2 |
| Ingress flag / validated landing-origin context / 시작 문항 정책 변경 | 2.1, 2.3, 2.4, 2.5, 2.6, 3.1, 3.2, 7.3, 11.2 |
| Staged entry expiry 변경 | 2.5, 1.3, 7.3, 11.2 |
| Runtime entry commit 정책 변경 | 2.4, 2.5, 3.2, 5.6, 7.3, 11.2 |
| Progress / revision / tail reset 정책 변경 | 2.9, 2.10, 3.3, 3.4, 7.3, 11.2 |
| Result-entry eligibility 정책 변경 | 2.10, 3.4, 3.5, 7.3, 11.2 |
| Result derivation loading / 최소 시간 변경 | 1.3, 3.6, 7.2, 7.3, 5.8, 11.2 |
| Back-from-loading 계약 변경 | 3.4, 3.7, 5.7, 7.2, 7.3, 11.2 |
| Commit-failure / Derivation-failure taxonomy 변경 | 5.5, 5.6, 5.7, 7.1, 7.3, 11.2 |
...
## 11. Acceptance / Traceability / Release Gate
...
7. **Question Model Integrity**: 모든 question이 정확히 2개 선택지. 각 question은 정확히 1개 axis만 평가.
...
18. **Telemetry Skeleton**: §8.1에 명시된 6개 hook 위치 확보 누락 `0건`.
