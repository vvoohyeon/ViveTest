...
| Result URL 구조 | self-contained payload (§4 참조) |
| Result URL 인코딩 | `/result/{testVariantId}?{URL-safe Base64 payload}`. variant는 path segment, 나머지(`scoringSchemaId`, `derivedType`, `scoreStats`)는 JSON → URL-safe Base64 → 키 없는 query string |
| 공유 URL 식별자 | `?share=y` 파라미터로 공유 여부 표시 |
...
**"결과 보기" CTA 제공 조건**:
- 마지막 문항에서 A/B 응답이 유효하게 선택된 상태면 "결과 보기" 액션을 제공한다.
...
**마지막 문항 응답 보존**:
- 시스템은 마지막 문항에서 사용자가 선택한 최신 A/B 응답을 임의로 제거하거나 변경하면 안 된다.
...
**URL 구조**:
```
/result/{testVariantId}?{base64Payload}
/result/{testVariantId}?{base64Payload}&share=y
```

예시:
```
/result/mbti?ejFEfkasdf...
/result/mbti?ejFEfkasdf...&share=y
```

**필드 분리 원칙**:
- `testVariantId`는 URL path segment로 표현한다. Base64 payload 내부에 중복 포함하지 않는다.
- 나머지 payload(`scoringSchemaId`, `derivedType`, `scoreStats`)는 JSON 직렬화 후 URL-safe Base64로 인코딩해 **키 없는 query string** 위치에 단독으로 추가한다.
- `share=y`는 공유 여부를 나타내는 별도 named 파라미터로, Base64 인코딩 대상에 포함하지 않는다.

**Base64 payload JSON 구조**:
```json
{
  "scoringSchemaId": "...",
  "derivedType": "INFJ",
...
- path의 `testVariantId`와 payload 내 `scoringSchemaId`가 가리키는 schema가 일치해야 한다. 불일치 시 §5.3 invalid payload로 처리한다.
- 공유 URL 생성 시 `&share=y`를 말미에 추가한다 (이번 단계 구현 제외, 구조만 예약).

### 4.2 결과 페이지 케이스 매트릭스

result 페이지 접근 시 아래 케이스 매트릭스에 따라 UX를 분기한다.

| 케이스 | Payload 유효성 | share=y | local storage 매칭 | UX | CTA |
|---|---|---|---|---|---|
| 1 | 유효 | 없음 | 무관 | 내 결과 UX | 다시하기 |
| 2 | 유효 | 있음 | 없음 | 공유받은 결과 UX | 나도 테스트하기 |
| 3 | 유효 | 있음 | 있음 | 내 결과 UX (share=y 무시) | 다시하기 |
| 4 | 무효 | 무관 | 무관 | 에러 렌더링 | 랜딩으로 돌아가기 |

**케이스 3**: history 미구현 상태이므로 이번 단계에서 케이스 2와 동일하게 처리한다. history 구현 단계에서 케이스 3 분기를 반드시 추가해야 한다 (AR-006).

CTA 동작:
- **다시하기**: `/test/{testVariantId}`로 이동. `instructionSeen`이 완료/timeout 시 리셋된 상태이므로, 테스트 페이지 진입 시 instruction overlay가 자동 표시된다. 별도 `/instruction` 라우트를 거치지 않는다.
- **나도 테스트하기**: `/test/{testVariantId}`로 이동. `instructionSeen`이 없는 첫 진입이므로, 테스트 페이지 진입 시 instruction overlay가 자동 표시된다.

**Verification**:
1. Automated: share=y 없는 유효 payload 접근 시 케이스 1 UX를 검증한다.
2. Automated: share=y 있는 유효 payload 접근 시 케이스 2 UX를 검증한다.
...
- path segment `testVariantId` 누락 또는 빈 값
- 키 없는 query string 누락 또는 URL-safe Base64 디코딩 실패
- JSON 파싱 실패
- 필수 필드 누락 (`scoringSchemaId`, `derivedType`, `scoreStats`)
- `derivedType` 토큰 길이가 path의 `testVariantId`로 식별된 schema의 `axisCount`와 불일치
- `scoreStats` 구조가 schema 선언과 불일치
- path의 `testVariantId`에 해당하는 schema를 찾을 수 없음
- `testVariantId`(path)와 `scoringSchemaId`(payload)가 가리키는 schema 불일치

에러 렌더링 UX: 랜딩으로 돌아가기 CTA 제공. 부분 렌더링을 금지한다.

**Verification**:
1. Automated: `testVariantId` path 누락, Base64 디코딩 실패, JSON 파싱 실패 각각에서 에러 렌더링을 검증한다.
2. Automated: 유효한 path + Base64 payload 조합에서 정상 렌더링을 검증한다.
3. Automated: 무효 payload에서 부분 렌더링 `0건`을 검증한다.
4. Automated: `testVariantId`–`scoringSchemaId` schema 불일치에서 에러 렌더링을 검증한다.
