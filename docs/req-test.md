...
- `type` segment 전체 길이는 `axisCount + sum(qualifierFields[i].tokenLength)`와 일치해야 한다. 불일치 시 §5.3 invalid payload로 처리한다.
- qualifier 값이 해당 `QualifierFieldSpec.values`에 없으면 §5.3 invalid payload로 처리한다.
- payload에 원문 질문/답변 텍스트, PII를 포함하면 안 된다.
- `variant` path segment으로 schema를 조회할 수 없는 경우 §5.3 invalid payload로 처리한다.
- `scoreStats` 구조는 `variant`로 식별된 schema의 scoring axes 선언과 일치해야 한다. 불일치 시 §5.3 invalid payload로 처리한다.
...
- **Affected Sections**: 1.3, 2.8, 2.9, 2.11, 4.1, 5.2, 5.3, 6.1, 10, 11.2
...
| `questionType` 정책 변경 (scoring/profile 분류) | 2.8, 2.9, 2.11, 5.2, AR-007, 11.2 |
| `qualifierFields` / `type` segment 구조 변경 | 1.3, 2.11, 4.1, 5.2, 5.3, 6.1, AR-007, 11.2 |
| `scoringMode` 정책 변경 (신규 모드 추가 포함) | 2.11, 5.2, 11.2 |
| `QualifierFieldSpec` 구조 변경 (key/values/tokenLength) | 2.11, 4.1, 5.3, 11.2 |
