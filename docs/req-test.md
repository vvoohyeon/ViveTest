...
- Landing preview payload(`previewQuestion`, `answerA`, `answerB`)는 Questions source의 **first scoring question (`scoring1`)**에서 파생한다. profile row는 preview source가 아니다.
...
- inline preview bridge는 source fixture에만 유지하고, consumer 노출은 `resolveTestPreviewPayload()` 경계로 제한한다.
- preview bridge는 source row의 first row가 아니라 **first scoring question (`scoring1`)**에서만 파생된다.
- `seq`는 source fixture 입력 전용이며 builder가 `questionType` 생성, source row order 보존, canonical index 재번호를 수행한 후 exported runtime registry에서는 제거한다.
...
- **Profile Question**: `questionType: 'profile'`로 정규화된 문항. axis 귀속이 없고 `scoreStats` 계산에서 제외된다. canonical `questions[]` 배열에 포함되며 progress와 `all-required-answered` 계산 대상이다.
...
**Progress 및 완료 게이팅**:
- `total question count`는 canonical `questions[]` 배열의 전체 길이다 (scoring + profile 포함).
- `all-required-answered`는 scoring 문항과 profile 문항 모두의 응답이 완료된 상태다.
- landing ingress flag 존재 시 landing에서 pre-answer된 `scoring1`의 canonical index는 answered로 계산한다. profile question이 존재하면 이 answered index는 canonical `1`이 아닐 수 있다.
...
- progress는 **canonical answered count / canonical total question count**를 기준으로 계산해야 한다.
- Ingress flag 존재 시 landing에서 pre-answer된 `scoring1`의 canonical index는 answered로 계산한다.
...
- user-facing `Q1/Q2/...`는 scoring order label일 뿐 progress denominator를 결정하지 않는다.
...
- 기존 run에 `scoring1` 응답이 이미 있으면: 새 ingress pre-answer를 무시하고 기존 응답 유지 (AR-005).
- 기존 run에 `scoring1` 응답이 없으면: 새 pre-answer를 `scoring1` 응답으로 저장.
- commit failure 시: §6.6 Commit-failure Error State로 처리.

**[처음부터 다시 하기]**:
- commit success 시: 기존 응답 데이터 즉시 완전 삭제 → `scoring1` pre-answer를 새 run의 `scoring1` 응답으로 저장 → runtime은 `q.1`이 있으면 `q.1`, 없으면 `scoring2`부터 새 run 시작. Instruction 표시 여부는 §3.6 `instructionSeen` 계약 준수.
- commit failure 시: §6.6 Commit-failure Error State로 처리.

**Verification**:
1. Automated: "이어서 하기" commit success 시 기존 응답 유지 + 마지막 응답 확정 문항 다음에서 재개를 검증한다.
2. Automated: "처음부터 다시 하기" commit success 시 기존 데이터 즉시 삭제 + landing ingress runtime start 규칙(`q.1` 우선, 없으면 `scoring2`)을 검증한다.
...
- progress는 현재까지 유효하게 응답된 **canonical question 수 / canonical total**을 기준으로 갱신되어야 한다.
...
- completion 판정은 scoring 문항만이 아니라 profile을 포함한 canonical 전체 문항 응답 완료를 기준으로 한다.
...
### AR-005 이어서 하기 시 `scoring1` 충돌 처리
- **Status**: ✅ 확정 (기존 run의 `scoring1` 응답 유지, 새 pre-answer 무시)
...
8. **Progress / Completion Gating**: progress가 canonical answered/total 기준이며 profile 문항을 포함한다. 미응답 canonical 문항 존재 시 completion 차단.
...
18. **Telemetry Skeleton**: §9.1에 명시된 6개 hook 위치 확보 누락 `0건`. `attempt_start` / `question_answered` index가 canonical 기준이며 user-facing Q label과 혼용되지 않음을 검증한다.
