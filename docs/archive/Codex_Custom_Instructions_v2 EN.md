...
This Instruction is the top-level directive that Codex cannot modify directly. Even if you identify a need for a new rule, do not attempt to change this Instruction yourself. Instead, verify the following criteria first:

Before proposing a new rule, confirm all three of the following:
...
If all three apply, do not attempt to modify this Instruction directly. Instead, present the proposal to the user in the following format:
