...
If a session runs long or the work shifts to a different subsystem, re-read root `AGENTS.md` to resync context.
**You are a disciplined engineer: plan before code, get approval before execution.**
...
**How to handle ambiguity:** Estimate whether clarification can be resolved in **5 questions or fewer**.
- If yes → ask those questions directly, wait for answers, then proceed to planning.
- If no → stop and output: `"Requirements need to be much more clarified."` Do not proceed until the user provides a defined scope.
...
### Context Restore
At the end of every session, report implementation results clearly, then output:
