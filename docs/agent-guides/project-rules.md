...
- **Token values live in `docs/design/ds/` (BQ-38).** `design.md` §5 states visual *intent*; `docs/design/ds/colors_and_type.css` holds the *realized* values, and `catalog-components.css` holds the catalog card system built from them. The repository owns both and pushes them one-way to the Claude Design project **VIVE Design System v2** (`cd630eec-25e4-4613-a58f-c671c80297ca`); the sync procedure and its operational traps are in `ds/SYNC.md`. **Nothing in `src/` consumes this layer** — it is documentation, and `catalog-components.css` is a specimen stylesheet that cannot be imported into the app (different class system, JS-measured geometry, a stage/frame/shell layering it flattens). Where a value in `ds/` differs from `design.md` §5, `ds/` is the realized value — but only the differences registered in BQ-38 are legitimate; register a new one before writing it. Open items carried by that bundle: M-01 (easing), D-01 (neutral-ramp temperature), D-02 (Pretendard not loaded), D-03 (accent hover/pressed), D-05 (four legacy values still rendering, page ground included).
- **Landing visual implementation:** lives in the `src/features/landing/grid/landing-grid-card*` family (orchestrator `landing-grid-card.tsx`; render branches `-normal-face` / `-expanded-body` / `-desktop-shell` / `-mobile-surfaces`; utility class strings `-classnames.ts`; public types `landing-card-contract.ts`) + `landing-grid-card.module.css`, using scoped `--normal-*` / `--expanded-*` tokens. Their *values* match the `design.md` semantic tokens; this coexistence is intentional — `design.md` is visual intent, `module.css` is implementation. Do not refactor scoped tokens as a side effect of unrelated work.
- **Landing keyboard entry:** owned by the skip link (`src/features/landing/shell/skip-to-content-link.tsx`), which is the first tab stop in document order and moves focus to `<main>`. Tab order is **always document order** — do not reintroduce state-dependent `tabIndex` on GNB controls, and do not intercept a document-level `Tab` to push focus into the GNB or the first card. Three mechanisms used to do exactly that and were removed together (BQ-40); `check-phase7-state-contracts.mjs` guards against their return.
