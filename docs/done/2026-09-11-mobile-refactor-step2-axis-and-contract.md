...
단위 0~4 전부 실행하고 착지했다. 커밋 여섯: 마커 · 단위 0 · 1-a · 1-b(1/2) · 1-b(2/2) · 단위 2 · 단위 3(1/2) · 3(2/2) · 단위 4.

### 이 계획서와 달라진 판정 넷

1. **§2-1 의 축 표가 명세와 어긋나 명세를 따랐다.** 표는 「확장 생명주기」를 통째로 `InputProfile` 에 배정하지만 명세 규칙 3 은 「형태는 공간이, 닫는 법은 입력이 정한다」이고 §2-11 이 표적 기기에 대해 같은 말을 한다. 이 계획서 자신이 「명세가 이긴다」고 적었으므로 형태는 폭에 남기고 **닫기 어포던스만** 입력 축으로 옮겼다. 완료 조건의 「닫기 X 와 backdrop」도 규칙 3 의 「보이는 X 를 두지 않는다」와 충돌해 같은 우선순위로 처분했다. 등재: BQ-40.
2. **§2-5 의 SSR 판본 선택은 「어느 쪽도 불필요」로 판정됐다.** 형태가 폭에 남으므로 SSR 이 뒤집을 것이 없다. 짝지은 실측 9 회에서 CLS 는 18 회 전건 0 이고 F2 지문이 네 커밋 내내 바이트 동일이다. 해법 ⒜·⒝ 어느 것도 넣지 않았다.
3. **단위 0 의 기준선 한 줄이 낡았다.** 「동의 배너 세 버튼의 인접 간격 0px」은 `4bdec85` 에서 재현되지 않는다(320~430px 실측 전부 WCAG 2.5.8 통과). 분석이 잰 것은 `a5aec95` 였다. 없는 결함에 시각 변경을 만들지 않았고, 실재한 결함(로고 링크 `68.06x24`) 하나만 고쳤다.
4. **§2-4 의 죽은 분기는 3 줄이 아니라 4 줄이고, 단위 3-2 의 기구는 둘이 아니라 셋이었다.** 넷째는 `use-hover-scroll-hold.ts` 의 해제 효과, 셋째 기구는 `use-keyboard-mode-tracker` 의 전역 `Tab` 가로채기다. 둘 다 이 계획서에 없다.

### 남긴 것

- **§8.5 전면 재작성은 하지 않았다**(계획대로) — `Escape` 한 줄만 열었다. 나머지는 step 3 의 IA 결정 뒤.
- **`data-card-viewport-tier` 속성 이름은 바꾸지 않았다**(계획대로).
- **`qa:visual:full` 을 실행하지 않았다.** baseline 파일은 `origin/main` 대비 변경 0 이다.
- **알고 남기는 것**: backdrop 은 `z-10`, GNB 는 `z-[1100]` 이라 최상단 GNB 띠에서는 빈 곳 탭이 닫지 않는다. 명세 규칙 1 의 층 순서는 step 3 소관이다.
- 함정 원장 `L33`(보고 호출 뒤에 붙인 계약 검사) · `L34`(클릭한 뒤 Tab 관용구) 등재.

### 게이트

`lint` · `typecheck` · `npm test`(90 files / 624 passed) · `qa:rules`(13/13) · `build` 초록. `test:e2e:smoke` **438 passed / 1 failed** — 붉음은 기계 종속 선재 `state-smoke:999`(`expanded-focus-shell.png`) 하나뿐이며 대조군 `4bdec85` 에서도 같다.
