...
**Reading this table after the theme cut.** Rows marked *Resolved* are closed in the system **and** in the product. Rows marked *Decided* are closed in the system only: the specimen and the tokens carry the answer, and the runtime change is step 5. D-08 is a *Proposal* — nothing about it has been adopted anywhere.
...
| D-05 | **Five of six closed by the theme cut, 2026-09-07.** The page ground, both title inks, the typeface and the mobile scrim now come from the system. **One remains and it changed shape:** `landing-grid-card.module.css:178` still puts `box-shadow: var(--card-shadow)` on the expanded card's root while focus is inside it, and after the cut that token resolves to `--shadow-rest` — a whisper — while the expanded lift is painted by a *different* element. Whether focus therefore flattens the card, or merely adds a faint second shadow, has **not been rendered**; it must be checked before it is fixed. | `preview/catalog-drift.html` |
| D-06 | **Decided, not yet implemented.** `design.md` §4.3 states one global wrapping rule; the product applies it to the card title and subtitle only on the mobile tier. Measured inside real cards at their real content boxes (258 / 363px): **seven of the ten fixture cards break Korean mid-word** in the two lines the clamp leaves visible, and the rule fixes every one without changing a single line count. The runtime change is step 5 — `landing-grid-card.module.css` applies the pair on the mobile tier only. | `preview/card-bilingual.html` |
...
| D-09 | **Decided at the system level, not yet in the product.** The mobile expanded sheet's close button is 40 × 40 against the 44 × 44 `design.md` §4.10 requires of it by name. `app-components.css` fixes the whole class of control rather than the instance — `.vt-pill--square` is `--tap-min` — so step 5 applies that class instead of resizing one button. | `preview/nav-mobile.html` |
