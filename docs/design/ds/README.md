...
**Reading this table.** Rows marked *Resolved* are closed in the system **and** in the product. **Step 5 has landed, so no row is waiting on a runtime change any more** — the *Decided* tier that used to hold rows between the two is empty and the rows that sat in it (D-06, D-09) now say where the product implements them. D-08 is a *Proposal* and is the one row still open: nothing about it has been adopted anywhere.
