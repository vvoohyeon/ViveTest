...
**Reading this table.** Rows marked *Resolved* are closed in the system **and** in the product. **Step 5 has landed, so no row is waiting on a runtime change any more** — the *Decided* tier that used to hold rows between the two is empty and the rows that sat in it (D-06, D-09) now say where the product implements them. Every row is now closed — D-08, the last one open, was adopted and drawn on 2026-09-10.
...
| D-02 | **Resolved by BQ-25; given a row here 2026-09-10.** The finding — "Pretendard is not loaded, so the product renders a fallback face" — has gone on being cited as open in `decision-register.md` and in session notes because it never had a row in this table, and **a number that is cited but not listed reads as unresolved forever**. The product loads it: `globals.css:31-37` declares the `@font-face` for Pretendard Variable (`font-weight: 45 920`, `font-display: swap`) from `/fonts/PretendardVariable.woff2`, that file ships in the repository at 2,057,688 bytes, and `--font-sans` names it first. | `preview/card-bilingual.html` |
...
| D-11 | **Resolved 2026-09-07.** `design.md` §7.6 contradicted `req-landing.md` §6.4 on navigation in three places, and all three were corrected in §7.6 itself, each annotated in place rather than settled here: the desktop settings trigger it forbade is now required (§6.4 specifies that layer's hover-open, close paths, `0px` hover gap and `<=1 frame` focus-out close across eight lines, four of them carrying automated checks, and a static pill cannot satisfy a contract that specifies its dropdown's geometry); the full-screen mobile menu is now the realized right-hand drawer over a scrim (§6.4 requires a `pointer down` **outside the panel** to begin the close, and a panel filling the viewport has no outside); and "Light active only" is gone, both themes being fully realized (D-10). Re-checked line by line against §6.4 on 2026-09-10. | `docs/done/2026-09-07-step3-U3-undesigned-surface-extraction.md` |
| D-05 | **Six of six closed. The last one was closed by rendering it (2026-09-07), and the answer was that there was nothing to fix.** The record had been written before the theme cut and was wrong in all three of its parts. The rule lives at `landing-grid-card.module.css:211` — not `:178` — and reads `box-shadow: var(--shadow-rest)`, not `var(--card-shadow)`: it is **warm, additive, and covered**. The card root carries `none` without focus and `rgba(30,26,22,0.04) 0 1px 2px` with it, while the expanded lift — `rgba(30,26,22,0.08) 0 12px 32px` plus a 1px edge, painted by a *different and wider* element (437.06 × 292 against the root's 397.33 × 292) — is identical in both states. So focus does not flatten the card; it adds a whisper the wider lift already covers. Re-measured independently on 2026-09-10 at 1440 × 980, light, DPR 2, isolating that one rule: its entire reach is a ~1.5px band at the card's bottom edge, moving it **1.7/255 per channel at the peak**, against **155** for an opaque control shadow at the same offset and blur — the sensor is ~35× more sensitive than the effect it measured. Below that band the focused and unfocused expanded states are pixel-identical. | `preview/catalog-drift.html` |
| D-06 | **Resolved 2026-09-07 (step 5).** `design.md` §4.3 states one global wrapping rule; the product had applied it to the card title and subtitle on the mobile tier only. Measured inside real cards at their real content boxes (258 / 363px): **seven of the ten fixture cards broke Korean mid-word** in the two lines the clamp leaves visible, and the rule fixed every one without changing a single line count. `landing-grid-card.module.css:128-132` now applies `word-break: keep-all` + `overflow-wrap: anywhere` to both slots at **every** tier — the rule sits outside every media query, confirmed by parsing the file's brace depth on 2026-09-10 rather than by reading the media blocks (L19). | `preview/card-bilingual.html` |
...
| D-08 | **Resolved 2026-09-10 — the proposal was adopted and drawn.** The catalog had shipped **one** illustration repeated across every card: only `qmbti` had an asset, and the generated fallback was *the same artwork*, so nothing distinguished the other seven. Ten compositions now ship at `public/landing-card-media/<variant>/thumbnail.svg`, one per variant in the registry (eight on the public landing, ten counting the QA-only `debug-sample` and `burnout-risk`), each drawn native to 640 × 240 in the single sage family and told apart by arrangement rather than hue: cadence · overlap · layers · balance · fork · signal · column · pending · probe · decay. The fallback is kept as a safety net for a future variant but is now deliberately **not** one of the compositions. **One deviation from rule ⑵, recorded here:** the ground is transparent rather than `--canvas`, so the card's own surface shows through and the drawing reads on both themes — a static SVG cannot follow the product's `data-theme` toggle, and a fixed light ground left every thumbnail a bright block on a dark card. `tests/unit/landing-card-thumbnails.test.ts` pins the set: every registry variant has an asset, no two assets are the same drawing (compared by content, not by path), every viewBox is `0 0 640 240`, the fallback differs from all of them, and no second hue appears. | `preview/thumb-proposal.html` |
| D-09 | **Resolved 2026-09-07 (step 5).** The mobile expanded sheet's close button was 40 × 40 against the 44 × 44 `design.md` §4.10 requires of it by name. `app-components.css` fixes the whole class of control rather than the instance — `.vt-pill--square` is `--tap-min` — and the card's own instance now carries the same floor: `landing-grid-card.tsx:270` sets `min-h-[var(--tap-min)] min-w-[var(--tap-min)]`, and `--tap-min` is `44px` (`globals.css:201`, verified 2026-09-10). | `preview/nav-mobile.html` |
| D-13 | **Open — the consent banner's two buttons do not weigh the same, and this row supersedes a decision made here.** 2026-09-08 this bundle registered the opposite: "배너의 거부는 수락과 같은 버튼 무게를 유지한다", reasoning that the two are symmetric consent answers. The visual and interaction specification of 2026-09-14 decided the other way with the user's confirmation — `Allow` is the filled CTA, `Deny` is a plain text button, `Preferences` is removed — and a later external review's recommendation to raise `Deny` back was rejected, so plain text is final. The system now says the specification's version. **Open until the product follows** (it still ships three buttons: measured 2026-09-16, `Accept all` 101x46 filled, `Deny` 69x46 outlined, `Preferences` 116x44). | `preview/banner-consent.html` |
| D-14 | **Open — the settings layer hung below its trigger, and the comment above it claimed otherwise.** The rule was `top: 100%`, which gives the `0px` hover gap `req-landing.md` §6.4 requires but not the overlap its own comment described ("anchored to the trigger's own box and overlaps it"). Three values were in play and all differed: this rule (below), the product (12px above the trigger and 15px beyond its right edge — measured 2026-09-16 at 1280x900, trigger `1212,10 44x44`, panel `932,-2 339x195`), and 규칙 8 (corners coincide). The system now states 규칙 8 and the specimen proves it — measured corner deviation `dx 0 · dy 0`. **Open until the product follows.** | `preview/nav-settings-layer.html` |
| D-15 | **Open — the system could not say "quiet", so it was read as requiring "loud".** §2-9 asks the desktop settings trigger to carry no border, no background, the default weight and the full language name, standing at the weight of the `History` and `Blog` links beside it. `.vt-pill` carries a border and `font-weight: 600`, and there was no modifier for the quiet case — a system that cannot express the specification's option will be implemented as its opposite, with a citation to back it up. `.vt-pill--quiet` added; the base pill is unchanged because `Menu`, `Back` and the drawer close are still correct as they are. **Open until the product follows** (it ships a bordered, filled 44x44 glyph pill — measured 2026-09-16). | `preview/nav-settings-layer.html` |
| D-16 | **Open — the theme swatch drew the double ring 규칙 5 forbids, and had no way to say "currently applied".** The previous rule was `0 0 0 2px var(--canvas), 0 0 0 4px var(--accent)`, which is exactly the "2px 흰 간격 + 2px sage" pair the rule calls 과하다 — and it was an honest mirror, the product draws it too (measured 2026-09-16). Two claims need two marks: selection is now a single sage ring, and "applied" is a badge dot (`.is-effective`) drawn with `position: absolute` so the mark never touches the layout (규칙 6). Without the second mark there is no way to show `System`, because the thing chosen and the thing applied are different. **Open until the product follows.** | `preview/nav-settings-layer.html` |
| D-17 | **Open — six surfaces step 3 builds had no definition here at all.** The sheet primitive, the instruction sheet, the `OPTED_OUT` notice row, the recall banner, the skip link and the landing page head. For the sheet the absence was total: `grabber`, `dvh`, `safe-area-inset` and `overscroll-behavior` appeared **zero times** across this bundle and across `src/**` (2026-09-16). `.vt-expanded--mobile` meanwhile describes the full-bleed in-flow model that step 3 §1 deletes — it is kept, and labelled, because it is still true of the product. Everything added under this row is marked `[intent]` in place and is promoted to realized by re-measuring after step 3 lands. | `preview/comp-sheet.html` · `preview/comp-notice.html` · `preview/banner-consent.html` · `preview/surface-landing-head.html` |
| D-18 | **Open — the sheet needs two durations the motion ladder deliberately does not have.** 명세 §3-4 asks for 260ms in and 220ms out; the ladder is 120 / 140 / 180 / 280. The absence is not an oversight: `colors_and_type.css`'s MOTION note retired exactly these two as "pre-rebaseline design tokens (which claimed 220ms / 260ms and were never implemented)". The specification has brought them back for a component that did not exist then. No token was invented here — the two values sit in `.vt-sheet` as literals with their source named. **The decision is whether the ladder gains `--dur-sheet-in` / `--dur-sheet-out` or the sheet snaps to 280 / 180**, and it belongs to step 3 §1. | `preview/comp-sheet.html` |
| D-19 | **Resolved 2026-09-16 here, open in the product — the skip link's own label was unreadable.** 명세 §2-13 specifies "`--accent` 바탕 + `--accent-fg` 잉크" and the product implements exactly that (`skip-to-content-link.tsx`). Measured on this specimen: **1.89:1 in light, 1.40:1 in dark** — under the 3:1 a non-text graphic needs, let alone the 4.5:1 its label needs. The cause is that `--accent-fg` (`#396050`) is the dark ink for use **on** a light sage fill, not an ink for use on the accent itself. The system already had the answer twenty lines above `.vt-btn--primary`: a fill sitting under its own label reads `--accent-solid` + `--fg-on-accent` (D-12). Now **5.09:1 light · 7.21:1 dark**. An accessibility control that cannot be read is the worst place for this, and only rendering it found it. | `preview/surface-landing-head.html` |
| D-20 | **Open — the bare `.t-caption` utility ships an ink that fails AA.** It sets `font` only, so the colour falls through to `--muted` (`#817a72`), which BQ-29 measured at 4.23:1. Every component in this bundle overrides it (`.vt-datarow__key`, `.vt-settings__label` both reach for `--muted-aa` and the latter explains why in place) — so the defect only surfaces in hand-written markup, which is exactly what a specimen is. It surfaced here and the sweep caught it. Either the utility should carry `--muted-aa`, or it should carry no colour at all and fail loudly; today it carries the one value the system already corrected. | `preview/comp-sheet.html` |
