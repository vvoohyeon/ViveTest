...
## 2026-09-03 — `§11 Response language`를 전역 계층으로 옮기고 여기서 제거

**배경.** 같은 날의 지침 4계층 재편에서 `CLAUDE.md §10`을 항목별로 판정할 때, `§11 Response language`(「사용자 메시지의 언어를 따른다」)만 판정이 갈렸다. 저장소와 무관한 규칙이므로 저장소 계약에 있을 이유가 없는데, 전역 `~/.claude/CLAUDE.md`에 해당 문장이 없어 지우면 실재하는 규칙이 사라진다. 그래서 그때는 남기고 「잘못 놓인 자리」라는 사실만 기록했다.

**선택과 근거.** 사용자 승인을 받아 규칙을 전역 `~/.claude/CLAUDE.md` §Deliverables 의 마지막 문단으로 올리고, 이 저장소에서는 제거했다. 그 절이 출력의 형식을 소유하므로 언어도 그 자리다. 전역 판본은 식별자·명령·경로·인용된 도구 출력은 원문 그대로 둔다는 절을 함께 얻었다 — 독자가 대조해야 하는 문자열을 번역하면 그 문자열이 쓸모를 잃기 때문이다.

**기각한 대안.** 양쪽에 두는 안 — 같은 규칙이 두 곳에 있으면 표류하고 인용은 패치되지 않은 쪽으로 옮겨 간다. 이 개편이 루트 `CLAUDE.md`를 삭제한 이유와 같은 이유이므로, 승격이 출처를 남기면 방금 없앤 이중 정본을 되살리는 셈이다.

**영향.** 이 저장소의 세션은 전역 계층에서 같은 규칙을 그대로 받는다. `AGENTS.md`는 211줄에서 207줄이 됐고 §10이 마지막 절이다. 전역 판본은 두 기계에 `config` 브랜치로 자동 동기화되므로 kospi-loop·ddokdan-web 세션도 함께 갖게 된다 — 이 저장소에만 있던 때보다 적용 범위가 넓어졌다.
