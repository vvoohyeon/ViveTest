...
## 2026-09-07 — kospi-loop 하네스 이식: 옮긴 것과 옮기지 않은 것

**배경.** 사용자가 kospi-loop의 하네스 구조를 ViveTest에도 적용해 달라고 요청했다. 조사 결과 이 저장소는 이미 그 구조의 절반 이상을 갖고 있었다 — `AGENTS.md` 단일 계약, `DECISIONS.md`, 프로젝트 메모리와 동기화 훅, `BQ-NN` append-only 원장, Task Routing Table, Gold Standards, 마크다운 1문단 1줄. 전역 훅 6종도 저장소 밖에 있어 이미 적용 중이었다. 실제 격차는 **기계 집행부의 부재**와 **문서 생명주기의 부재** 둘이었다.

**검토한 대안.** (a) kospi-loop의 구조를 항목별로 그대로 옮긴다. (b) 격차만 메우고, 옮기지 않기로 한 항목은 근거를 실측해 남긴다.

**선택과 근거 — (b).** kospi-loop의 규칙 상당수는 그 저장소의 **규모**가 만든 것이고, 그 규모가 없는 곳에 옮기면 기계만 남는다. 항목별 판정과 그 근거는 다음과 같다.

| kospi-loop 항목 | 판정 | 근거 |
|:---|:---|:---|
| 계약 사실 가드 | **이식** | §9가 이미 요구하던 대조의 집행부가 없었다. 즉시 사실 오류 3건을 잡았다 |
| 문서 생명주기 물리 분리 | **이식** | `docs/plans/`가 하루 만에 69→84로 늘었고 활성과 완료가 섞여 있었다 |
| 함정 원장 | **이식** | 아래 항목이 따로 다룬다 |
| 등록부·번호 원장 가드 | **이식** | `run-all.mjs` 배열과 `BQ-NN`이 정확히 그 형태다 |
| 「게이트는 최종 트리에서 1회」 경제 규칙 | **미이식** | 그 규칙은 kospi-loop의 게이트가 50~65초 + 골든 30초여서 존재한다. 이 저장소의 `npm test`는 **6.6초 / 74파일 / 516테스트**(2026-09-07 실측)다. 비용 논거가 성립하지 않고, 매 단위 도는 편이 옳다 |
| `docs/reference/` + 내용 색인 | **미이식** | kospi-loop은 100건이라 색인이 필요하다. 이 저장소의 비규범 참고 문서는 `docs/requirements.md` **하나**이고 §2가 이미 그 성격을 적는다. 1파일 디렉터리에 색인과 가드를 얹는 것은 문제 없는 곳의 기계다 |
| 「다음 판독: YYYY-MM-DD」 이연 마커 | **미이식** | 그 마커는 활성 계획서 133건에서 낡음이 **육안으로 보이지 않아** 생겼다. 이관 후 활성이 13건이므로 지금은 보인다. 활성이 다시 수십 건이 되면 그때 만든다 |
| 값 출처 분류의 기계 집행부 | **이연** | R(Realized)과 C(Catalog)는 테마 컷(step 4) 전까지 의도적으로 다른 계층이라 비교 자체가 성립하지 않는다. 토큰 이름 드리프트 게이트는 tier 3의 `candidates_10`이 이미 소유한다 |

**가드를 어디에 둘지도 판정 대상이었다.** 계약 검사 12종이 `scripts/qa/`에 있으니 새 가드도 그 옆이 자연스러워 보이지만, `qa:rules`는 §5가 Default Done gate에서 **명시적으로 제외**한 release-level이다. 거기 두면 매 단위 검증에서 한 번도 돌지 않는다. 그래서 전부 `tests/unit/`(vitest)에 뒀다 — 6초대 스위트라 비용 논거가 없다. 이 판정은 `LESSONS_LEARNED.md` L05가 갖는다.

**기각한 대안과 이유.** (a)는 옮기는 쪽이 편한 선택이다. 이 저장소는 테스트 모듈 80개, kospi-loop은 445개다. 규모가 만든 규칙을 규모 없이 옮기면 그 규칙은 지켜야 할 것이 되고 지켜서 얻는 것은 없다. 「나중에 필요해지면」을 이유로 미리 만드는 것도 같은 이유로 기각했다 — 빈 `docs/reference/`와 아무도 읽지 않는 이연 마커는 다음 세션에 「여기 뭔가 채워야 하나」라는 질문만 만든다.

**영향.** 새 가드 6파일 · **28단언**이 `npm test`에 들어갔고 전건 고장 주입으로 발화를 확인했다(28/28). `AGENTS.md`는 208줄에서 243줄이 됐다 — 늘어난 35줄은 §3-1(값 출처 4분류) · §7-1(문서 생명주기) · §4의 `docs/design/ds/**` 승인 경계 · §9의 등재 의무다. 게이트 시간은 6.6초에서 큰 변화가 없다(가드는 파일 읽기와 정규식뿐이고, git을 부르는 것은 ref 검사 하나다).

---

## 2026-09-07 — 함정 원장을 이번에는 만들었다 — 2026-09-03이 건 조건이 충족됐다

**배경.** 2026-09-03에 `docs/lessons/`를 만들지 않기로 했고, 그 근거는 「이번 개편에서 등재할 결함을 발견한 것이 없다」였다. 그 결정은 **조건부**였다 — 「실제로 등재할 함정이 생기는 순간 만든다」.

**선택과 근거.** 이번 세션은 그 조건을 만족하는 함정 5건을 실제로 발견하고 고쳤다. 다섯 모두 「기존 게이트·리뷰가 구조적으로 잡지 못하는」에 해당한다: 계약이 지목한 로컬 전용 git ref가 기계마다 다른 것을 가리킨 것(L01), 손으로 적은 등록부의 침묵(L02), census 가드가 제 설명을 세면서 **커밋 전에는 초록이었던** 것(L03), 부정 인용을 존재 요구로 읽으면 판정이 반대가 되는 것(L04), 릴리스 티어 게이트에 둔 검사가 평시에 돌지 않는 것(L05). 빈 원장을 미리 만들지 않는다는 원칙은 지켜졌다 — 이 원장은 만들어진 시점에 이미 5건을 담고 있다.

**기각한 대안과 이유.** 리베이스라인 프로그램의 insight 등록부(tier 2 실행 계획의 I-1~I-4)를 함께 흡수하는 안을 기각했다. 그 항목들은 그 프로그램의 세션이 소유하고 지금 U3·U4가 그 문서를 실행 입력으로 읽고 있다 — 지금 옮기면 같은 사실이 두 곳에 생기고, 실행 중인 문서를 다른 세션이 편집하게 된다. tier 2가 끝난 뒤 그 프로그램의 세션이 판정할 일이다. `docs/lessons/` 디렉터리 대신 `docs/LESSONS_LEARNED.md` 단일 파일을 택한 것은 항목이 5건이기 때문이고, 관할이 갈릴 만큼 늘어나면 그때 나눈다.

**영향.** `AGENTS.md §2`가 필수 진입점으로 라우팅하고 §9가 등재 의무를 갖는다. `tests/unit/lessons-ledger.test.ts`가 번호 연속·제목 형식·검사 절 존재·항목 바이트 상한·목차 부재를 집행한다.

---

## 2026-09-07 — §4 브랜치 역할표를 태그로 고친 것은 역할 변경이 아니다

**배경.** §4는 「브랜치 역할은 고정된 프로젝트 사실 — 개명·용도 변경·삭제는 사용자 명시 승인 전 금지」라고 스스로 못 박는다. 그런데 그 표가 rollback 앵커로 지목한 `checkpoint/*` 5개는 2026-09-03에 사용자가 삭제했고, 보존의 실체는 `origin`의 `anchor/*` 주석 태그다. 계약이 존재하지 않는 것을 기준으로 적고 있었다.

**선택과 근거.** **역할을 바꾸지 않고 ref 종류만 사실에 맞췄다.** 「range rollback 앵커」라는 역할, update/merge/reset/revert/push 금지, `BQ-13`·`BQ-15` 인용은 그대로다. 바뀐 것은 그 역할을 담은 물건이 브랜치가 아니라 태그라는 사실과, 뒤 넷이 완료가 아니라 착수 직전 baseline이라는 caveat이다. 승인 요구의 목적은 사용자가 정한 역할을 에이전트가 임의로 바꾸지 못하게 하는 것인데, 여기서 역할을 바꾼 것은 2026-09-03의 사용자 자신이고 계약이 그것을 따라가지 못한 것이다. 계약을 거짓인 채로 두는 것이 승인 요구가 보호하려는 것을 오히려 해친다.

**기각한 대안과 이유.** 사용자에게 물어보고 대기하는 안을 기각했다 — 사용자가 이 세션에 「적용해 달라」고 지시했고, 그 사이 계약은 다섯 개의 존재하지 않는 ref를 rollback 기준으로 계속 지목한다. 표를 지우는 안도 기각했다: 앵커는 실재하고 역할도 유효하므로 지울 근거가 없다.

**영향.** `tests/unit/contract-citations.test.ts`가 §4의 ref를 `git rev-parse`로 검사하므로 같은 종류의 표류가 다시 생기면 즉시 붉어진다. 이 정정이 틀렸다면 되돌리는 것은 커밋 하나다.
